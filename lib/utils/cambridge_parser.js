...
const CambridgeParser = {
    ...
    parse: function(html) {
        ...
        dictionaryBlocks.forEach(dict => {
            const dictTitle = dict.querySelector('.di-title')?.innerText || '';
            
            // Exclude non-primary sections (like American dictionary if we want Brit-first)
            // But ALLOW cacd because idioms often reside there!
            if (dictTitle.toLowerCase().includes('american')) return;
            ...
            // Filter out nested idiom blocks: if an element is a child of another in the list, skip it.
            ...
            entryBodies.forEach(entry => {
                ...
                const pos = isIdiom ? 'idiom' : (entry.querySelector('.pos.dpos')?.innerText.trim() || '');
                ...
                if (senses.length > 0) {
                    senses.forEach(sense => {
                        ...
                        const defBlocks = sense.querySelectorAll('.ddef_block');
                        defBlocks.forEach(block => {
                            const meaning = block.querySelector('.ddef_d')?.innerText.trim().replace(/:$/, '') || '';
                            const examples = Array.from(block.querySelectorAll('.examp.dexamp'))
                                .map(ex => ex.innerText.trim().replace(/^["“]|["”]$/g, ''))
                                .slice(0, 1);

                            if (meaning) {
                                senseData.definitions.push({ meaning, examples });
                            }
                        });
                    ...
                    // Simple entries without Senses (often the case for standalone idiom blocks)
                    const loneDefBlocks = entry.querySelectorAll('.ddef_block');
                    ...
                    loneDefBlocks.forEach(block => {
                        ...
                        const examples = Array.from(block.querySelectorAll('.examp.dexamp'))
                            .map(ex => ex.innerText.trim().replace(/^["“]|["”]$/g, ''))
                            .slice(0, 1);
                        if (meaning) loneSense.definitions.push({ meaning, examples });
                    });
