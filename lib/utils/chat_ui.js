...
class LuminaChatUI {
    ...
    appendChunk(chunk, skipScroll = false) {
        ...
        if (!answerContentDiv.__isRich) {
            if (typeof marked !== 'undefined') {
                const fastText = newText.replace(/<think>[\s\S]*?(?:<\/think>|$)/gi, '');
                ...
                const fastText = newText.length < 1000 ? newText.replace(/<think>[\s\S]*?(?:<\/think>|$)/gi, '') : newText.substring(0, 1000) + "...";
    ...
    static async injectCopyButtons(container) {
        ...
        const COPY_SVG = `<span class="lumina-svg-icon lumina-icon-copy" aria-hidden="true"></span>`;
        const CHECK_SVG = `<span class="lumina-svg-icon lumina-icon-copy-alt" aria-hidden="true"></span>`;
