...
class LuminaChatUI {
    ...
    appendQuestion(text, images = [], options = {}) {
        ...
        if (images && images.length > 0) {
            ...
            // Store for history serialization
            questionDiv.dataset.images = JSON.stringify(images);
        }

        // Add text
        if (text) {
            // Strip $container tokens from DISPLAY only (sent text remains untouched)
            const displayText = text.replace(/[("'\[]*\$Container[)"'\]]*\s*/gi, '').trim();
            
            // Store RAW text for fast serialization (avoiding DOM scans)
    ...
    appendChunk(chunk, skipScroll = false) {
        ...
        // --- Fast path: show raw text immediately ---
        // We only do this if we haven't rendered rich markdown yet, to avoid flickering
        // and to keep CPU usage low on high-frequency chunks.
        ...
        if (!answerContentDiv.__isRich) {
            // Very cheap: just show content if it's still plain text
            // Skip expensive regex/think-tag stripping in the high-frequency fast path
            // (Throttled _doRender handles the clean-up every 400ms)
            if (newText.length < 1000) {
                // For small text, we can afford a quick cleanup
                let fastText = newText.replace(/<think>[\s\S]*?(?:<\/think>|$)/gi, '');
                answerContentDiv.textContent = fastText || '...';
            } else {
                // For large text, just show trailing snippet or full text minus think tags at start
                answerContentDiv.textContent = newText.substring(0, 1000) + "...";
            }
        }

        // --- Slow path: full markdown render, throttled to 400ms ---
        // Heavy (marked.parse + innerHTML) — runs infrequently so Windows CPUs aren't starved.
        // Input events and text selection stay responsive between renders.
    ...
    /**
     * Give the new entry a min-height equal to the visible viewport so it fills the
     * screen immediately (Gemini-style). Previous entries lose their min-height and
     * collapse to natural content height. Scroll to align the previous separator at top.
     *
     * @param {HTMLElement} entry - The new entry element
     * @param {boolean} smooth - Unused (kept for call-site compatibility)
     * @param {number} forcedStartScroll - Unused
     */
    setInitialEntryHeight(entry, smooth = false, forcedStartScroll = null) {
        ...
        if (viewportHeight > 0) {
            // Constant 10px spacing instead of reading separator margins (layout thrash)
            ...
            setTimeout(() => this.setInitialEntryHeight(entry, smooth, forcedStartScroll), 80);
            return;
        }
    ...
    renderFilePreviews() {
        ...
        this.attachedFiles.forEach((file, index) => {
            ...
            if (file.isImage) {
                ...
                if (file.mimeType.startsWith('audio/')) icon = '<svg viewBox="0 0 24 24" width="24" height="24"><path fill="currentColor" d="M12 3v9.28c-.47-.17-.97-.28-1.5-.28C8.01 12 6 14.01 6 16.5S8.01 21 10.5 21c2.31 0 4.2-1.75 4.45-4H15V6h4V3h-7z"/></svg>';
                if (file.mimeType.startsWith('video/')) icon = '<svg viewBox="0 0 24 24" width="24" height="24"><path fill="currentColor" d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>';
                if (file.mimeType === 'application/pdf') icon = '<svg viewBox="0 0 24 24" width="24" height="24"><path fill="currentColor" d="M20 2H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-8.5 7.5c0 .83-.67 1.5-1.5 1.5H9v2H7.5V7H10c.83 0 1.5.67 1.5 1.5v1zm5 2c0 .83-.67 1.5-1.5 1.5h-2.5V7H15c.83 0 1.5.67 1.5 1.5v3zm4-3H19v1h1.5V11H19v2h-1.5V7h3v1.5zM9 9.5h1v-1H9v1zM4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm10 5.5h1v-3h-1v3z"/></svg>';

                content = `<div class="lumina-file-icon">${icon}<span class="lumina-file-name">${file.name}</span></div>`;
            }
    ...
    gatherMessages() {
        ...
        entries.forEach(entry => {
            ...
            if (questionEl) {
                ...
                const images = questionEl.dataset.images ? JSON.parse(questionEl.dataset.images) : [];
                messages.push({
                    role: 'user',
                    text: questionText,
                    files: images
    ...
    setupInputBar() {
        ...
        const checkExpand = () => {
            ...
            if (modelSelectorEl) {
                ...
                // Measure both placeholder AND value as single unwrapped lines.
                // Use the wider of the two so that a narrow container that expanded
                // due to placeholder stays expanded even as the user types short text.
                ...
                const singleLineWidth = Math.max(
                    measureSingleLine(input.placeholder),
                    measureSingleLine(input.value)
                );
        ...
        chrome.storage.local.get(['customSources'], (res) => {
            const sources = res.customSources || (window.LUMINA_DEFAULTS ? window.LUMINA_DEFAULTS.customSources : []);
            // Cache so the input listener can do keyword detection without async
            ...
            // Prevent duplicate insertion if called multiple times
            if (toolsDropdown.querySelector('.lumina-web-sources-header')) return;

            // Divider
            const divider = document.createElement('div');
            divider.style.cssText = 'height:1px; background:var(--border-color); margin:4px 0; opacity:0.5;';
            toolsDropdown.appendChild(divider);

            // Header
            const header = document.createElement('div');
            header.className = 'lumina-web-sources-header';
            ...
            header.textContent = 'Web Sources';
            toolsDropdown.appendChild(header);

            sources.forEach(source => {
                const btn = document.createElement('button');
                btn.className = 'lumina-tool-item';
                btn.innerHTML = `<span>${this.escapeHTMLAttr(source.name)}</span>`;
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();

                    // Always activate websource mode and show toggle
                    removeActiveModes();
                    ...
                    const text = input.value.trim();
                    if (text) {
                        // If there's already text, process immediately
                        this.openWebSource(source, text);
                        input.value = '';
                        input.style.height = 'auto';
                        removeActiveModes();
                    } else {
    ...
    /**
     * Open a web source URL in an iframe as a chat entry.
     * @param {Object} source - The source configuration {name, url, css}
     * @param {string} text - The search term
     */
    openWebSource(source, text) {
        if (!source || !text) return;

        // Strip $Container and surrounding punctuation/whitespace from search term
        const cleanText = text.replace(/[("'\[]*\$Container[)"'\]]*\s*/gi, '').trim();
        if (!cleanText) return;

        const encodedText = encodeURIComponent(cleanText);
        const finalUrl = source.url.replace('{{str}}', encodedText);

        // Append the "question"
        this.appendQuestion(`${source.name}: ${text}`, [], { entryType: 'tool' });

        // Create the answer container
        const answerDiv = this.createAnswerDiv();
        answerDiv.innerHTML = ''; // Clear Thinking indicators

        // source.height is a percentage (20–100) of the scroll container height
        const heightPct = Math.min(100, Math.max(20, source.height || 80));
        const scrollContainer = this.getScrollContainer();
        const containerH = scrollContainer ? (scrollContainer.clientHeight || scrollContainer.offsetHeight) : 0;
        // Fallback to a reasonable px value if the container isn't laid out yet
        const iframeHeight = containerH > 0 ? Math.round(containerH * heightPct / 100) : Math.round(400 * heightPct / 100);

        const container = document.createElement('div');
        container.className = 'lumina-websource-container is-loading';
        container.dataset.sourceId = source.id;
        container.dataset.sourceHeightPct = String(heightPct);
        container.dataset.sourceUrl = finalUrl;
        // Outer container: position relative + no overflow so ::after glow is NOT clipped
        container.style.cssText = `width:100%; margin:0; position:relative; height:${iframeHeight}px; border-radius:12px;`;

        // Inner wrapper: clips the iframe to rounded corners
        const innerWrapper = document.createElement('div');
        innerWrapper.className = 'lumina-websource-inner';
        innerWrapper.style.cssText = `width:100%; height:100%; border-radius:12px; overflow:hidden; position:relative;`;

        const iframe = document.createElement('iframe');
        const startTime = Date.now();

        // Register injection BEFORE setting src so webNavigation fires with data ready
        if (typeof chrome !== 'undefined' && chrome.runtime && (source.css || source.selector || (source.zoom && source.zoom !== 100))) {
        ...
        // Remove loading state when iframe is ready
        iframe.onload = () => {
        ...
        iframe.src = finalUrl;
        iframe.title = source.name;
        iframe.loading = 'lazy';   // Performance
        iframe.referrerPolicy = 'no-referrer-when-downgrade'; // Privacy
        ...
        if (source.css) {
            ...
            style.className = 'lumina-websource-custom-css';
            style.textContent = `.lumina-websource-container { ${source.css} }`;
        ...
        // Final margin adjustment once everything is added to the DOM
        requestAnimationFrame(() => {
            if (this.currentEntryDiv) this.adjustEntryMargin(this.currentEntryDiv, 'none');
            this.finishAnswer();
        });
    }
    ...
    getInputState() {
        return {
            ...
            isWebSourceMode: !!this._pendingWebSource,
            pendingWebSource: this._pendingWebSource || null,
    ...
    restoreInputState(state) {
        ...
        if (state.isProofreadMode) {
            ...
            const toolItem = queryInPopup('[data-action="translate"]');
            if (toolItem) toolItem.classList.add('active');
        } else if (state.isWebSourceMode && state.pendingWebSource) {
            this._pendingWebSource = state.pendingWebSource;
            const toggle = queryInPopup('#websource-toggle') || queryInPopup('.lumina-websource-toggle');
            if (toggle) {
                const label = toggle.querySelector('.tool-label');
                if (label) label.textContent = state.pendingWebSource.name;
    ...
    _handleSubmit() {
        ...
        if (this.options.onSubmit) {
            this.options.onSubmit(text, this.attachedFiles, {});
        }
    ...
    static async processContainer(container) {
        ...
        // 6. Revive Web Source iframes (register with background for CSS injection)
        await LuminaChatUI.reviveWebSources(container);
    }

    /**
     * Update existing Web Source containers when settings change.
     * @param {Array} customSources - New sources from storage
     */
    static updateWebSourceDimensions(customSources) {
        if (!customSources || !Array.isArray(customSources)) return;
        const containers = document.querySelectorAll('.lumina-websource-container');
        if (containers.length === 0) return;

        containers.forEach(container => {
            const sid = container.dataset.sourceId;
            const source = customSources.find(s => s.id === sid);
            if (!source) return;

            // 1. Update Height
            const newPct = Math.min(100, Math.max(20, source.height || 80));
            // To get accurate height, we need the scroll container. 
            // Since this is static, we try to find the nearest scroll-content ancestor 
            // or use a generic selector.
            const scrollContainer = container.closest('.lumina-chat-scroll-content') ||
                container.closest('.lumina-chat-history') ||
                document.querySelector('.lumina-chat-scroll-content');

            if (scrollContainer) {
                const containerH = scrollContainer.clientHeight || scrollContainer.offsetHeight;
                if (containerH > 0) {
                    const newH = Math.round(containerH * newPct / 100);
                    container.style.height = `${newH}px`;
                    container.dataset.sourceHeightPct = String(newPct);
                }
            }

            // 2. Zoom & Selector & CSS are handled by background.js pushing to iframes.
            // But we can also trigger a re-message to be sure.
            const finalUrl = container.dataset.sourceUrl;
            if (finalUrl && typeof chrome !== 'undefined' && chrome.runtime) {
                chrome.runtime.sendMessage({
                    action: 'prepare_iframe_injection',
                    frameUrl: finalUrl,
                    css: source.css || '',
                ...
                // Also trigger immediate injection actions if we're in the right tab
                chrome.runtime.sendMessage({ action: 'inject_iframe_css', frameUrl: finalUrl, css: source.css || '' }).catch(() => { });
                chrome.runtime.sendMessage({ action: 'inject_iframe_zoom', frameUrl: finalUrl, zoom: source.zoom || 100 }).catch(() => { });
                if (source.selector) {
                    chrome.runtime.sendMessage({ action: 'inject_iframe_selector', frameUrl: finalUrl, selector: source.selector }).catch(() => { });
                }
            }
        });
    }
    ...
    /**
     * Find all Web Source containers and re-register them with background script.
     * Needed when history is restored or pages are reloaded.
     * @param {HTMLElement} container
     */
    static async reviveWebSources(container) {
        if (!container) return;
        const webSources = container.querySelectorAll('.lumina-websource-container');
        if (webSources.length === 0) return;

        try {
            const data = await chrome.storage.local.get(['customSources']);
            const customSources = data.customSources || [];

            webSources.forEach(ws => {
                const sourceId = ws.dataset.sourceId;
                const finalUrl = ws.dataset.sourceUrl;
                if (!sourceId || !finalUrl) return;
            ...
            console.error('[Lumina] Failed to revive Web Sources:', e);
        }
    }
...
if (typeof window !== 'undefined') {
    ...
    // Listen for storage changes to update Web Source dimensions in real-time
    chrome.storage.onChanged.addListener((changes, areaName) => {
        if (areaName === 'local' && changes.customSources) {
            LuminaChatUI.updateWebSourceDimensions(changes.customSources.newValue);
        }
    });

    // Also listen for direct broadcast messages from options page
    chrome.runtime.onMessage.addListener((request) => {
        if (request.action === 'update_websource' && request.source) {
            // Find existing container with this source ID and update it
            const containers = document.querySelectorAll(`.lumina-websource-container[data-source-id="${request.source.id}"]`);
            if (containers.length > 0) {
                LuminaChatUI.updateWebSourceDimensions([request.source]);
            }
        }
    });
}
