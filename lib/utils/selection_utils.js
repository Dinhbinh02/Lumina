window.LuminaSelection = {
    ...
    async init(options = {}) {
        ...
        this.btn.innerHTML = `
            ...
            <div class="lumina-comment-part"><span class="lumina-svg-icon lumina-icon-plus" aria-hidden="true"></span><span>Add Comment</span></div>
            <div class="lumina-ask-part"><span>Ask Lumina</span></div>
        `;
    ...
    cleanup() {
        ...
        if (this.shadowRoot) {
            this.shadowRoot.querySelectorAll('#lumina-ask-selection-btn, #lumina-ask-input-popup, .lumina-overlay-backdrop').forEach(el => el.remove());
        }
    ...
    show(x, y, text, range) {
        ...
        if (proofreadNode) {
            // Only allow comment mode for the LATEST proofread block in the container
            const container = proofreadNode.closest('.lumina-chat-scroll-content') || 
                              proofreadNode.closest('.lumina-chat-history') || 
                              document.body;
            const allProofreads = container.querySelectorAll('.lumina-proofread-editable');
            if (allProofreads.length > 0 && allProofreads[allProofreads.length - 1] === proofreadNode) {
