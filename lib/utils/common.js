class LuminaChatUI {
        ...
        if (!this._renderPending) {
            ...
            setTimeout(() => {
                if (this.currentAnswerDiv) {
                    ...
                    this._pendingRenderSkipScroll = false;
                    this._doRender(this.currentAnswerDiv, shouldSkipScroll);
                } else {
                    this._renderPending = false;
                    this._pendingRenderSkipScroll = false;
                }
    ...
    _handleQuestionRecheck(userInput, editable) {
        ...
        // Remove the old answer and web-search artifacts, but keep the separator so the
        // regenerated answer reuses the same entry structure.
    ...
    clearAnswer(entryDiv) {
        if (!entryDiv) return;
        const answerDiv = entryDiv.querySelector('.lumina-chat-answer') || entryDiv.querySelector('.lumina-answer-versions');
    ...
    _updateActionBtnState() {
        ...
        if (this.isGenerating) {
            ...
            actionBtn.innerHTML = `<svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>`;
    ...
    _handleQuestionRecheck(newText, questionContent, isRegenerate = false) {
        if (typeof this.options.onSubmit === 'function') {
            const entry = questionContent.closest('.lumina-dict-entry');
            const entryId = entry ? entry.dataset.entryId : null;
            this.options.onSubmit(newText, [], { isRecheck: true, isRegenerate, entryId });
        }
    }
