class LuminaChatUI {
    ...
    initListeners(container) {
        ...
        container.addEventListener('keydown', (e) => {
            const editable = e.target.closest('.lumina-chat-question .lumina-question-content[contenteditable="true"]');
        ...
        // 4. Mousedown Delegation (Open/focus Question edit mode)
        container.addEventListener('mousedown', (e) => {
            const questionBubble = e.target.closest('.lumina-chat-question');
            if (questionBubble) {
                if (e.target.closest('.lumina-question-pin-btn')) return;

                const editable = questionBubble.querySelector('.lumina-question-content[contenteditable="true"]');
                if (editable) {
                    if (!e.target.closest('.lumina-question-edit-toolbar')) {
                        e.preventDefault();
                        editable.focus();
                ...
                if (!questionBubble.classList.contains('is-editing')) {
                    e.preventDefault();
                    this.enterQuestionEditMode(questionBubble);
                }
            }
        });
    ...
    static applyFontSize(container, size) {
        if (!size) return;
        const fontSizeStr = size + 'px';
        if (container) {
            container.style.setProperty('font-size', fontSizeStr, 'important');
        } else if (document.body) {
            document.body.style.setProperty('font-size', fontSizeStr, 'important');
        }
        document.documentElement.style.setProperty('--lumina-fontSize', fontSizeStr, 'important');
    }
    ...
    _updatePinnedQuestionChip() {
        ...
        if (!this._pinnedQuestionChipEl) {
            ...
            chip.innerHTML = '<span class="lumina-svg-icon lumina-icon-thumbtack" aria-hidden="true"></span><span class="lumina-web-chip-label"></span>';
    ...
    appendQuestion(text, images = [], options = {}) {
        ...
        if (text) {
            const baseText = displayTextOverride || text;
        ...
        entries.forEach(entry => {
            ...
            if (questionEl) {
                ...
                if (isProofreadEntry) {
                    ...
                    if (answerEl) {
                        const editableContent = answerEl.classList.contains('is-editing')
            ...
            if (answerEl) {
                const answerText = answerEl.classList.contains('is-editing')
    ...
    _initContextMenu() {
        ...
        this.container.addEventListener('contextmenu', (e) => {
            if (this.historyEl?.querySelector('.is-editing')) {
    ...
    regenerateEntry(entry) {
        ...
        const questionContent = questionDiv.querySelector('.lumina-question-content')
            || (questionDiv.querySelector('.lumina-question-content')?.hasAttribute('contenteditable'))
            || questionDiv;
    ...
    enterQuestionEditMode(questionDiv) {
        ...
        let contentDiv = questionDiv.querySelector('.lumina-question-content');
        if (!contentDiv) {
        ...
        questionDiv.classList.add('is-editing', 'lumina-fade-in');
        ...
        const focusHandler = (e) => {
            if (e.target.closest('.lumina-edit-btn')) return;
            contentDiv.focus();
            try {
                const selection = window.getSelection();
                const range = document.createRange();
        ...
        questionDiv.addEventListener('click', focusHandler);
        questionDiv.__questionEditFocusHandler = focusHandler;
    ...
    exitQuestionEditMode(questionDiv, save = false) {
        ...
        const contentDiv = questionDiv.__questionEditContentDiv || questionDiv.querySelector('.lumina-question-content') || questionDiv;
        ...
        if (questionDiv.__questionEditFocusHandler) {
            questionDiv.removeEventListener('click', questionDiv.__questionEditFocusHandler);
        }
        ...
        if (save) {
            ...
            questionDiv.classList.remove('is-editing', 'lumina-fade-in');
            if (row) row.classList.remove('lumina-question-row-editing');
            if (toolbar) toolbar.remove();
            if (contentDiv) contentDiv.removeAttribute('contenteditable');

            this._handleQuestionRecheck(newText, contentDiv || questionDiv);
        } else {
        ...
        delete questionDiv.__questionEditFocusHandler;
    }

    cancelAllQuestionEdits() {
        if (!this.historyEl) return;
        const editingQuestions = Array.from(this.historyEl.querySelectorAll('.lumina-chat-question.is-editing'));
    ...
    enterAnswerEditMode(answerDiv) {
        ...
        answerDiv.removeAttribute('contenteditable');
        ...
        answerDiv.classList.add('is-editing', 'lumina-fade-in');
        contentDiv.setAttribute('contenteditable', 'true');
        try {
            contentDiv.focus({ preventScroll: true });
        } catch (e) {
        ...
        answerDiv.addEventListener('click', focusHandler);
        answerDiv.__answerEditFocusHandler = focusHandler;
    ...
    exitAnswerEditMode(answerDiv, save = false) {
        ...
        if (answerDiv.__answerEditKeyHandler) {
            contentDiv.removeEventListener('keydown', answerDiv.__answerEditKeyHandler);
        }
        if (answerDiv.__answerEditFocusHandler) {
            answerDiv.removeEventListener('click', answerDiv.__answerEditFocusHandler);
        }
        ...
        answerDiv.classList.remove('is-editing', 'lumina-fade-in');
    ...
    cancelAllAnswerEdits() {
        if (!this.historyEl) return;
        const editingAnswers = Array.from(this.historyEl.querySelectorAll('.lumina-chat-answer.is-editing'));
    ...
    serializeHistoryHTML() {
        ...
        editingAnswers.forEach((liveAnswer) => {
            ...
            clonedAnswer.classList.remove('is-editing', 'lumina-fade-in');
            clonedAnswer.removeAttribute('contenteditable');
        ...
        editingQuestions.forEach((liveQuestion) => {
            ...
            clonedQuestion.classList.remove('is-editing', 'lumina-fade-in');
