class LuminaChatUI {
    ...
    initListeners(container) {
        ...
        // 1. Keydown Delegation (Enter to re-submit)
        ...
        // 2. Mousedown Delegation (Focus Question bubble area)
    ...
    appendQuestion(text, images = [], options = {}) {
        ...
        if (text) {
            const displayText = displayTextOverride || text.replace(/[("'\[]*\$Container[)"'\]]*\s*/gi, '').trim();
        ...
        if (!skipMargin) {
            ...
            requestAnimationFrame(() => {
                this.setInitialEntryHeight(this.currentEntryDiv, false, preAppendScroll);
            });
        } else {
    ...
    _handleQuestionRecheck(question, editable) {
        ...
        if (this.options.onSubmit) {
            this.options.onSubmit(question, [], { isRecheck: true });
        }
    }
    ...
    scrollToBottom(targetElement = null) {
        LuminaChatUI.scrollToBottom(this.getScrollContainer(), targetElement);
    }
    ...
    async _handleContextMenuAction(action, answer) {
        ...
        switch (action) {
            ...
            case 'edit': {
                ...
                if (questionDiv) {
                    const editable = questionDiv.querySelector('[contenteditable="true"]');
                    if (editable) {
                        editable.focus();
                        // Scroll question into view if needed
