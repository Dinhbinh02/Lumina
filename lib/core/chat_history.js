        ...
        while (i < chatData.messages.length) {
                    ...
                    if (answerMsg.versions && answerMsg.versions.length > 1) {
                        ...
                        answerMsg.versions.forEach((versionContent, idx) => {
                            ...
                            if (versionContent.trim().startsWith('<')) {
                                if (typeof window !== 'undefined' && window.parseLuminaAlerts) {
                                    answerDiv.innerHTML = window.parseLuminaAlerts(versionContent);
                                } else {
                        ...
                        if (answerMsg.content.trim().startsWith('<')) {
                            if (typeof window !== 'undefined' && window.parseLuminaAlerts) {
                                answerDiv.innerHTML = window.parseLuminaAlerts(answerMsg.content);
                            } else {
            ...
            if (msg.type === 'answer') {
                ...
                if (msg.content.trim().startsWith('<')) {
                    if (typeof window !== 'undefined' && window.parseLuminaAlerts) {
                        answerDiv.innerHTML = window.parseLuminaAlerts(msg.content);
                    } else {
