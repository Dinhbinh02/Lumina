...
const ChatHistoryManager = {
    ...
    async saveCurrentChat(historyEl = null, optionalSessionId = null, sparkId = null, force = false, extraSettings = null) {
        ...
        try {
            ...
            const optimizedMessages = messages.map(msg => {
                if (msg.type === 'question' && Array.isArray(msg.images)) {
                    return {
                        ...msg,
                        images: msg.images.map(img => {
                            if (typeof img === 'object' && img && img.attachmentId) {
            ...
            if (sortedIds.length > this.MAX_HISTORIES) {
                ...
                deletedIds.forEach(id => {
                    ...
                    contentKeys.push(`spotlight_history_${id}`);
                });
    ...
    async restoreChat(chatData, historyContainer = null) {
        ...
        while (i < chatData.messages.length) {
                    ...
                    if (answerMsg.versions && answerMsg.versions.length > 1) {
                        ...
                        answerMsg.versions.forEach((versionContent, idx) => {
                            ...
                            if (displayContent.trim().startsWith('<')) {
                                ...
                                let content = safelyFormatContentSpacing(displayContent);
                        ...
                        const displayContent = answerMsg.content.replace(/<think>[\s\S]*?(?:<\/think>|$)/gi, '').trim();
                        if (displayContent.trim().startsWith('<')) {
                            answerDiv.innerHTML = displayContent;
                        } else if (typeof marked !== 'undefined') {
                            let content = safelyFormatContentSpacing(displayContent);
            ...
            if (msg.type === 'answer') {
                ...
                const displayContent = msg.content.replace(/<think>[\s\S]*?(?:<\/think>|$)/gi, '').trim();
                if (displayContent.trim().startsWith('<')) {
                    answerDiv.innerHTML = displayContent;
                } else if (typeof marked !== 'undefined') {
                    let c = safelyFormatContentSpacing(displayContent);
    ...
    async deleteChat(sessionId) {
        try {
            ...
            await chrome.storage.local.remove([`lumina_session_${sessionId}`, `spotlight_history_${sessionId}`]);

            // OPFS Cleanup: Delete all stored files for this session ID
    ...
    async clearAllHistory() {
        try {
            ...
            // OPFS Cleanup: Delete all local files
            chrome.runtime.sendMessage({ action: 'get_stored_files' }, (response) => {
                if (response && response.success && Array.isArray(response.files)) {
        ...
        try {
            ...
            if (deletedSessionIds.length > 0) {
                ...
                deletedSessionIds.forEach(id => {
                    ...
                    sessionKeys.push(`spotlight_history_${id}`);
                });
                await chrome.storage.local.remove(sessionKeys);

                // OPFS Cleanup: Trigger full orphans cleanup
