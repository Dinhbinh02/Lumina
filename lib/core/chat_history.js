...
function safelyFormatContentSpacing(text) {
    if (!text || typeof text !== 'string') return '';
    // Mask markdown image/link patterns so that base64 data and URLs don't get corrupted by spacing replacements
    const masks = [];
    const maskedText = text.replace(/(!?\[.*?\]\(.*?\))/g, (match) => {
        masks.push(match);
        return `__LUMINA_MASK_${masks.length - 1}__`;
    });
    
    let processed = maskedText
        .replace(/([a-zà-ỹ])(\d)/g, '$1 $2')
        .replace(/(\d)([a-zà-ỹ])/g, '$1 $2');
        
    processed = processed.replace(/__LUMINA_MASK_(\d+)__/g, (match, idx) => {
        return masks[parseInt(idx)];
    });
...
const ChatHistoryManager = {
        ...
        while (i < chatData.messages.length) {
                    ...
                    if (answerMsg.versions && answerMsg.versions.length > 1) {
                        ...
                        answerMsg.versions.forEach((versionContent, idx) => {
                            ...
                            if (versionContent.trim().startsWith('<')) {
                                ...
                                let content = safelyFormatContentSpacing(versionContent);
                        ...
                        if (answerMsg.content.trim().startsWith('<')) {
                            ...
                            let content = safelyFormatContentSpacing(answerMsg.content);
            ...
            if (msg.type === 'answer') {
                ...
                if (msg.content.trim().startsWith('<')) {
                    ...
                    let c = safelyFormatContentSpacing(msg.content);
    ...
    async deleteChat(sessionId) {
        try {
            ...
            // Run cache cleanup asynchronously to purge orphaned images
    ...
    async cleanupHistoryByAge() {
        try {
            ...
            const months = settings.historyRetentionMonths !== undefined ? parseInt(settings.historyRetentionMonths, 10) : 3;
            ...
            if (deletedSessionIds.length > 0) {
                ...
                const sessionKeys = deletedSessionIds.map(id => `lumina_session_${id}`);
                await chrome.storage.local.remove(sessionKeys);

                // Run image cache cleanup immediately to prune orphaned images
    ...
    async cleanupImageCache() {
        try {
            ...
            const sessionKeys = Object.keys(allStorage).filter(key => key.startsWith('lumina_session_'));
            
            for (const key of sessionKeys) {
                const messages = allStorage[key];
                if (Array.isArray(messages)) {
                    messages.forEach(msg => {
                        if (msg && msg.content && typeof msg.content === 'string') {
                            const matches = msg.content.match(/cacheKey:([a-zA-Z0-9_\-]+)/g);
                            if (matches) {
                                matches.forEach(match => {
                                    referencedKeys.add(match.substring('cacheKey:'.length));
                                });
                            }
                        }
                        if (msg && Array.isArray(msg.versions)) {
                            msg.versions.forEach(vContent => {
                                if (vContent && typeof vContent === 'string') {
                                    const matches = vContent.match(/cacheKey:([a-zA-Z0-9_\-]+)/g);
                                    if (matches) {
                                        matches.forEach(match => {
                                            referencedKeys.add(match.substring('cacheKey:'.length));
                                        });
            ...
            const imageCacheKeys = Object.keys(allStorage).filter(key => key.startsWith('lumina_img_cache_'));
            const keysToRemove = imageCacheKeys.filter(key => {
                const cacheKey = key.substring('lumina_img_cache_'.length);
                return !referencedKeys.has(cacheKey);
            });

            if (keysToRemove.length > 0) {
                console.log(`[Lumina Cache] Purging ${keysToRemove.length} orphaned image cache keys from local storage:`, keysToRemove);
                await chrome.storage.local.remove(keysToRemove);
            }
        } catch (error) {
            console.error('[Lumina Cache] Error cleaning up image cache:', error);
        }
    }
};
...
ChatHistoryManager.migrateIfNeeded().then(async () => {
    ...
    await ChatHistoryManager.cleanupImageCache();
});
