            ...
            if (msg.type === 'question') {
                ...
                if (visibleImages.length > 0) {
                    ...
                    if (hydratePromises.length > 0) {
                        const writePromise = Promise.all(hydratePromises).then(() => {
                            questionDiv.dataset.images = JSON.stringify({
                                compact: true,
                                count: visibleImages.length,
                                files: visibleImages
                        ...
                        allHydratePromises.push(writePromise);
                    }
    ...
    <div class="lumina-canvas-card-info">
      ...
      <div class="lumina-canvas-card-meta">${displayType} • ${timeStr}</div>
        ...
        if (processPromises.length > 0) {
            ...
            // Execute highlighting and formatting asynchronously in the background so the chat is displayed immediately
            Promise.all(processPromises).catch(err => console.error('Error during async chat processing:', err));
        }
        ...
        if (allHydratePromises.length > 0) {
            await Promise.all(allHydratePromises);
        }
    },
