...
function mergeSparks(local, remote, useLocalSettings, lastSynced = 0) {
    if (!local || typeof local !== 'object') return remote || {};
    if (!remote || typeof remote !== 'object') return local || {};
    const merged = {};
    const allIds = new Set([...Object.keys(local), ...Object.keys(remote)]);
    for (const id of allIds) {
        const localSpark = local[id];
        const remoteSpark = remote[id];
        if (localSpark && remoteSpark) {
...
class SyncManager {
    ...
    async syncData(isAuto = false) {
        ...
        try {
            ...
            let fileId = null;
            let remoteFile = null;
            let lastSyncMd5 = null;
            try {
                remoteFile = await this.findBackupFile(token);
                if (remoteFile) {
                    fileId = remoteFile.id;
                    const syncMeta = await chrome.storage.local.get(['last_sync_md5']);
                    lastSyncMd5 = syncMeta.last_sync_md5;
                    if (!lastSyncMd5 || remoteFile.md5Checksum !== lastSyncMd5) {
                        remoteBackup = await this.downloadBackup(token, fileId);
                    } else {
                        console.log('[Sync] Remote file has not changed (MD5 matches), skipping download.');
                    }
                }
            } catch (err) {
                if (err.message === 'UNAUTHORIZED') {
                    ...
                    if (remoteFile) {
                        ...
                        if (!lastSyncMd5 || remoteFile.md5Checksum !== lastSyncMd5) {
                            ...
                            console.log('[Sync] Remote file has not changed (MD5 matches) after re-auth, skipping download.');
                        }
                    }
                } else {
                    throw err;
            ...
            if (typeof LuminaChatDB !== 'undefined') {
                try {
                    ...
                    for (const s of Object.values(sessions)) {
                        if (s && s.id) {
                            sessionsObj[s.id] = s;
                            const messages = await LuminaChatDB.getMessages(s.id).catch(() => []);
                            localData[`lumina_session_${s.id}`] = messages;
                        }
                    }
            ...
            for (const key of allKeys) {
                ...
                if (key === 'providers') {
                    ...
                    mergedData[key] = mergeSparks(localData[key], remoteData[key], useLocalSettings, lastSynced);
            ...
            // Notify UI to refresh the recent chats list immediately
            try {
            ...
            let newUploadedMd5 = null;
            if (fileId) {
                newUploadedMd5 = await this.updateBackupFile(token, fileId, JSON.stringify(payload));
            } else {
                newUploadedMd5 = await this.createBackupFile(token, JSON.stringify(payload));
            }
            await chrome.storage.local.set({
