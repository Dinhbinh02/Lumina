...
function mergeArrayByIds(localArr, remoteArr, lastSynced = 0) {
    const localList = Array.isArray(localArr) ? localArr : [];
    const remoteList = Array.isArray(remoteArr) ? remoteArr : [];
    
    const localMap = new Map(localList.filter(item => item && item.id).map(item => [item.id, item]));
    const remoteMap = new Map(remoteList.filter(item => item && item.id).map(item => [item.id, item]));
...
class SyncManager {
    ...
    mergeSyncData(localData, remoteData) {
        ...
        for (const key of allKeys) {
            ...
            } else if (key === 'lumina_notes_collections' || key === 'lumina_notes_items') {
                mergedData[key] = mergeArrayByIds(localData[key], remoteData[key], lastSynced);
