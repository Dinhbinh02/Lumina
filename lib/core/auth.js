...
function mergeSparks(local, remote) {
...
class AuthService {
    ...
    async init() {
        try {
            // Clean up legacy credentials and PKCE keys
            await chrome.storage.local.remove(['client_id', 'client_secret', 'refresh_token', 'pkce_verifier']);
        ...
        try {
            ...
            for (const key of allKeys) {
                ...
                if (key === 'providers') {
                    ...
                    mergedData[key] = mergeSparks(localData[key], remoteData[key]);
