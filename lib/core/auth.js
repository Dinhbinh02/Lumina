...
async function refreshAccessToken(clientId, clientSecret, refreshToken) {
    ...
    if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        const err = new Error(errData.error_description || errData.error || "Failed to refresh token");
        err.isAuthError = (res.status === 400 || res.status === 401);
        throw err;
    }
    const data = await res.json();
    return {
...
class AuthService {
            ...
            chrome.storage.local.get(["google_oauth_token", "google_oauth_token_time", "refresh_token", "client_id", "client_secret"], async (result) => {
                ...
                if (result.refresh_token) {
                    try {
                        ...
                        if (err.isAuthError) {
                            console.error("[Auth] Refresh token is invalid/revoked. Clearing token storage:", err);
                            await chrome.storage.local.remove(["google_oauth_token", "google_oauth_token_time", "refresh_token", "google_user_info"]);
                            result.refresh_token = null;
                        } else {
                            console.warn("[Auth] Temporary error refreshing token (possibly network offline):", err);
                            reject(err);
                            return;
                        }
                    }
    ...
    fallbackToWebAuthFlowPKCE(resolve, reject, clientId, clientSecret, challenge) {
        ...
        chrome.identity.launchWebAuthFlow({
            ...
            chrome.storage.local.get(["pkce_verifier", "refresh_token"], async (result) => {
                ...
                try {
                    ...
                    if (code) {
                        try {
                            ...
                            const newRefreshToken = tokenRes.refreshToken || result.refresh_token;
                            const storageData = {
        ...
        try {
            ...
            for (const sid of allSessionIds) {
                ...
                if (localS && remoteS) {
                    ...
                    // Always keep local sessions to prevent accidental data loss.
                    // Instead of deleting them, we merge and keep them in the synchronized state.
                    mergedSessions[sid] = localS;
                    if (`lumina_session_${sid}` in localData) mergedData[`lumina_session_${sid}`] = localData[`lumina_session_${sid}`];
                    if (`spotlight_history_${sid}` in localData) mergedData[`spotlight_history_${sid}`] = localData[`spotlight_history_${sid}`];
                } else if (remoteS) {
