...
class AuthService {
    ...
    async getAuthToken(interactive = false, forceRefresh = false) {
        // If we are on non-Chrome browsers (Edge, Brave, Opera, Firefox, etc.) where native getAuthToken is not supported,
        // we use launchWebAuthFlow and cache the token in chrome.storage.local.
...
class SyncManager {
    ...
    async syncData(isAuto = false) {
        ...
        try {
            ...
            ].includes(k) || k.includes('_inst_') || k.startsWith('pending_sidepanel_query_');
            ...
            // Build a Set of all referenced attachment IDs from all active sessions
            ...
            // Clean up orphaned attachments from local DB
