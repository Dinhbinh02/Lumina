window.LuminaSelection = {
    ...
    currentHighlightColor: '#fff59d',
    showExtraColors: false,
    ANNOTATION_COLORS: ['#fff59d', '#ffcc80', '#f48fb1', '#ce93d8', '#90caf9'],
    ...
    renderDefaultActionBar() {
        ...
        if (this.annotationMode) {
            // ONLY show Clear button and all 5 colors
            html += `
            ...
            // Highlight button with currentHighlightColor
            html += `
    ...
    showAnnotationMenu(targetElOrRange, id, currentColor) {
        ...
        this.currentHighlightColor = currentColor || '#fff59d';
        this.showExtraColors = false;
    ...
    setScrollLock(lock) {
        const isSpotlight = document.body.classList.contains('lumina-spotlight-page') || window.location.pathname.includes('spotlight.html');
