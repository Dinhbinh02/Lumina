window.LuminaSelection = {
    ...
    expandToWordBoundaries() {
        ...
        // Only expand if we are dealing with text nodes
        if (startNode.nodeType !== Node.TEXT_NODE || endNode.nodeType !== Node.TEXT_NODE) return;

        // PRE-TRIM: If selection starts/ends with whitespace, pull boundaries in.
        // This is CRITICAL for cases where the browser's native double-click selects "word ".
        // If we don't pull it in, our "endsWithSymbol" expansion logic would greedily 
        // expand until the NEXT whitespace, eating the following word.
        ...
        // Unified word char check (covers EN, VI, and numbers)
        // \u00C0-\u1EF9 is the standard range for Vietnamese Latin extensions
        ...
        while (startOffset > 0) {
            ...
            if (/\s/.test(prevChar)) break; // Always stop at whitespace

            if (startsWithWordChar) {
                // If we started with a letter, stop at first non-letter symbol (like parenthesis)
                if (!isWordChar(prevChar)) break;
            }
            // If we started with a symbol (like '('), keep going until whitespace
            startOffset--;
        }
        ...
        while (endOffset < endText.length) {
            ...
            if (/\s/.test(nextChar)) break; // Always stop at whitespace

            if (endsWithWordChar) {
                // If we ended with a letter, stop at first non-letter symbol (like ')')
                if (!isWordChar(nextChar)) break;
            }
            // If we ended with a symbol (like '?'), keep going until whitespace
            endOffset++;
        }

        // Final trimming of spaces (in case user selected them or expansion reached them)
        ...
        try {
            ...
            // Update the stored range and text in LuminaSelection
            this.range = newRange;
        ...
        this.btn.addEventListener('click', (e) => {
            ...
            if (highlightBtn) {
                if (window.LuminaAnnotation && this.range) {
                    ...
                    // CRITICAL: Clone range and SAVE before APPLYING.
                    // Applying mutations changes the DOM and collapses the range, 
                    // which would break serialization if done in the wrong order.
                    const rangeToHighlight = this.range.cloneRange();
                    window.LuminaAnnotation.saveHighlight(rangeToHighlight, color, id);
                    window.LuminaAnnotation.applyHighlight(rangeToHighlight, color, id);
                }
                const selection = window.getSelection();
        ...
        try {
            ...
            const getOffset = (targetNode, targetOffset) => {
                ...
                while (walker.nextNode()) {
                    ...
                    if (offset > 10000) break; // Safety break
                }
                return offset;
            };
    ...
    restoreSelectionFromOffsets(container, offsets) {
        ...
        try {
            ...
            // Silently fail if DOM structure changed too much
        }
    }
};
