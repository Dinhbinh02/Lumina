...
window.LuminaDictionaryPopup = {
    ...
    async fetchData(source = this.currentSource) {
        ...
        try {
            ...
            const payload = source === 'images' ? { action, keyword: this.currentWord } : { action, word: this.currentWord };
    ...
    renderImages(images) {
        ...
        if (!images || images.length === 0) {
            scrollArea.innerHTML = `<div class="lumina-dict-empty">No images found for "${this.currentWord}".</div>`;
            return;
        }
        ...
        // Add click to enlarge or open in new tab
            ...
            <div class="lumina-dict-popup-senses">
                ${this.currentSource === 'ai' && entry.definitions ? 
                    entry.definitions.map((def, idx) => `
                        <div class="lumina-dict-popup-meaning ai-custom">
                            <div class="lumina-dict-meaning-header">
                                <span class="lumina-dict-meaning-keywords">${def.keywords || ''}</span>
                            </div>
                            ${def.explanation ? `<div class="lumina-dict-meaning-explanation">${def.explanation}</div>` : ''}
                        </div>
                    `).join('')
