...
window.LuminaDictionaryPopup = {
    ...
    async show(word, options = {}) {
        ...
        if (this.currentSource === 'translate') {
            ...
            const sources = ['dictionary', 'images'];
    ...
    setupAudioListeners(container) {
            ...
            btn.onclick = async (e) => {
                ...
                if (url) {
                    ...
                    try {
                        ...
                        const speed = settings.audioSpeed || 1.1;
        ...
        const fcBtns = container.querySelectorAll('.lumina-dict-add-fc-btn');
        fcBtns.forEach(btn => {
            btn.onclick = (e) => {
                e.stopPropagation();
                const { word, definition } = btn.dataset;
                chrome.runtime.sendMessage({
                    action: 'add_to_flashcards',
                    word,
                    definition
    ...
    buildEntryHTML(entry, word) {
        ...
        (entry.senses || []).forEach(s => {
            (s.definitions || []).forEach(d => {
                definitionsList.push(`${d.meaning}`);
            });
        });
        const definitionStr = definitionsList.join('\n');
        ...
        return `
            <div class="lumina-dict-popup-meta">
                <div class="lumina-dict-header-row">
                    ...
                    <button class="lumina-dict-add-fc-btn" title="Add to Flashcards" 
                        data-word="${entry.word || word}" 
                        data-definition="${definitionStr.replace(/"/g, '&quot;')}"
                        style="background:none; border:none; cursor:pointer; font-size:16px; margin-left:auto; padding:2px; display:inline-flex; align-items:center;">➕</button>
                </div>
