...
window.LuminaDictionaryPopup = {
    ...
    getFallbackSource(source) {
        const sources = ['cambridge', 'oxford'];
    ...
    async show(word, options = {}) {
        ...
        let initialSource = options.source || 'cambridge';
        if (initialSource === 'freedict' || initialSource === 'ai') {
            initialSource = 'cambridge';
        }
        ...
        if (this.currentSource === 'translate') {
            ...
            const sources = ['cambridge', 'oxford', 'images'];
