function parseLuminaAlerts(html) {
    if (!html) return html;

    const standaloneRegex = /^<p>(?:<strong>)?\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*(?:<\/strong>)?(?::)?\s*<\/p>/i;
    const inlineRegex = /^<p>(?:<strong>)?\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*(?:<\/strong>)?(?::)?\s*(?:\s*<br\s*\/?>)?/i;
    ...
    return html.replace(/<blockquote[^>]*>([\s\S]*?)<\/blockquote>/gi, (match, content) => {
        ...
        const standaloneMatch = cleanContent.match(standaloneRegex);
        if (standaloneMatch) {
            type = standaloneMatch[1].toUpperCase();
            innerContent = cleanContent.replace(standaloneRegex, '').trim();
        } else {
            const inlineMatch = cleanContent.match(inlineRegex);
            if (inlineMatch) {
                type = inlineMatch[1].toUpperCase();
                innerContent = cleanContent.replace(inlineRegex, '<p>').trim();
            }
        }
        ...
        if (type) {
            let iconSvg = '';
            if (type === 'NOTE') {
                iconSvg = `<svg class="lumina-alert-icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>`;
            } else if (type === 'TIP') {
                iconSvg = `<svg class="lumina-alert-icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A5 5 0 0 0 8 8c0 1 .4 2.5 1.5 3.5.7.8 1.3 1.5 1.5 2.5"></path><path d="M9 18h6"></path><path d="M10 22h4"></path></svg>`;
            } else if (type === 'IMPORTANT') {
                iconSvg = `<svg class="lumina-alert-icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>`;
            ...
            const formattedTitle = type.charAt(0) + type.slice(1).toLowerCase();
            return `<div class="lumina-alert lumina-alert--${type.toLowerCase()}">` +
                   `<div class="lumina-alert-title">${iconSvg}<span>${formattedTitle}</span></div>` +
                   `<div class="lumina-alert-content">${innerContent}</div>` +
                   `</div>`;
        }
...
if (typeof window !== 'undefined') {
    window.parseLuminaAlerts = parseLuminaAlerts;
}

if (typeof marked !== 'undefined' && marked.parse) {
    const originalMarkedParse = marked.parse;
    marked.parse = function(src, options) {
        let html = originalMarkedParse.apply(this, arguments);
        return parseLuminaAlerts(html);
    };
}
...
class LuminaChatUI {
    ...
    getLoadingHTML() {
        return `<div class="lumina-thinking"><div class="lumina-loading-spinner"></div><span>Thinking</span></div>`;
    }
