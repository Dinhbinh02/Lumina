        ...
        const visibleImages = Array.isArray(images)
            ? images.filter((item) => {
                ...
                if (item.fileUri && item.fileUri.startsWith('local-db://')) {
                    ...
                    item.attachmentId = 'file_att_' + Date.now() + '_' + index + '_' + Math.random().toString(36).substr(2, 5);
                }
                if (item.dataUrl && item.dataUrl.startsWith('data:')) {
                    ...
                    if (!item.fileUri || !item.fileUri.startsWith('local-db://')) {
                        const storeBlob = async () => {
                            ...
                            if (item.isImage && dataUrlToUse.length > 2 * 1024 * 1024) {
                                dataUrlToUse = await this._compressImage(dataUrlToUse, 2048, 2048, 0.9);
  ...
  <div class="lumina-canvas-card-left">
    ...
    async addFile(file) {
        ...
        if (this._isPdfFile(file)) {
            ...
            setTimeout(async () => {
                try {
                    const rawPdf = await this._prepareRawFileAttachment(file);
                    if (rawPdf) {
                    ...
                    const derivedFiles = await this._extractPdfAsAttachments(file);
    ...
    _compressImage(dataUrl, maxWidth = 2048, maxHeight = 2048, quality = 0.9) {
        return new Promise((resolve) => {
            const img = new Image();
            img.onload = () => {
                let width = img.width;
    ...
    async _extractPdfAsAttachments(file) {
    ...
    renderFilePreviews() {
        ...
        visibleEntries.forEach(({ file, index }) => {
            ...
            itemDiv.className = `lumina-preview-item ${file.isImage ? 'is-image' : 'is-file'} ${file.status === 'uploading' ? 'is-uploading' : ''}`;
        ...
        input.addEventListener('paste', async (e) => {
            ...
            for (const item of items) {
                if (item.type.startsWith('image/')) {
                    ...
                    const compressedDataUrl = await this._compressImage(rawDataUrl, 2048, 2048, 0.9);
...
// Dynamic Script & CSS lazy loading helpers
