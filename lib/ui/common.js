            ...
            code(token) {
                ...
                if (lang === 'mermaid') {
                    const escaped = text
                        .replace(/&/g, '&amp;')
                        .replace(/"/g, '&quot;');
                    return `<div class="lumina-mermaid-wrapper is-loading" data-mermaid="${escaped}"><div class="lumina-media-skeleton lumina-mermaid-skeleton"></div></div>`;
                }
                return false;
            },
            image(token) {
...
// ── Mermaid Diagram Rendering (Lazy-loaded + IntersectionObserver) ──────────

let _luminaMermaidLoadPromise = null;
let _luminaMermaidInitialized = false;
let _luminaMermaidIdCounter = 0;

function _loadMermaidScript() {
    if (_luminaMermaidLoadPromise) return _luminaMermaidLoadPromise;
    _luminaMermaidLoadPromise = new Promise((resolve, reject) => {
        if (typeof mermaid !== 'undefined') { resolve(); return; }
        const url = (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getURL)
            ? chrome.runtime.getURL('lib/vendor/mermaid.min.js')
            : '../../lib/vendor/mermaid.min.js';
        const script = document.createElement('script');
        script.src = url;
        script.onload = resolve;
        script.onerror = () => { _luminaMermaidLoadPromise = null; reject(new Error('Failed to load mermaid')); };
        document.head.appendChild(script);
    });
    return _luminaMermaidLoadPromise;
}

function _initMermaidTheme() {
    if (_luminaMermaidInitialized || typeof mermaid === 'undefined') return;
    ...
    _luminaMermaidInitialized = true;
}

function _alignEdgeLabelsWithClusters(svgEl) {
    if (!svgEl) return;
    try {
        const clusters = svgEl.querySelectorAll('.cluster');
        const edgeLabels = svgEl.querySelectorAll('.edgeLabel');
        if (clusters.length === 0 || edgeLabels.length === 0) return;

        const clusterBoxes = Array.from(clusters).map(c => {
            ...
            if (!fill || fill === 'none') {
                try {
                    fill = window.getComputedStyle(rect).fill || '';
                } catch (e) {}
            }
            if (!fill || fill === 'none') {
                fill = '#ffffff';
        ...
        edgeLabels.forEach(el => {
            ...
            for (const cb of clusterBoxes) {
                if (centerX >= cb.x && centerX <= cb.x + cb.w &&
                    centerY >= cb.y && centerY <= cb.y + cb.h) {
            ...
            if (matchingCluster) {
                const bg = matchingCluster.fill;
                const targets = el.querySelectorAll('p, span, div, rect');
                targets.forEach(t => {
                    t.style.setProperty('background-color', bg, 'important');
                    t.style.setProperty('background', bg, 'important');
                    if (t.tagName.toLowerCase() === 'rect') {
                        t.style.setProperty('fill', bg, 'important');
                    }
                });
            }
        });
    } catch (e) {
        console.warn('[Lumina Mermaid] Failed to align edge labels:', e);
    }
}

async function _renderMermaidWrapper(wrapper) {
    // Already started or done
    if (wrapper.classList.contains('is-loading-started')) return;
    wrapper.classList.add('is-loading-started');

    let source = (wrapper.getAttribute('data-mermaid') || '')
        .replace(/&amp;/g, '&')
        .replace(/&quot;/g, '"')
        .replace(/&#39;/g, "'")
        .replace(/&lt;/g, '<')
    ...
    // Automatically convert vertical flowchart/graph layout to horizontal (LR)
    if (source.match(/^\s*(flowchart|graph)\s+(TD|TB|BT|RL)\b/i)) {
        source = source.replace(/^\s*(flowchart|graph)\s+(TD|TB|BT|RL)\b/i, '$1 LR');
    }
    // Convert direction TB/TD inside subgraphs to LR
    source = source.replace(/\bdirection\s+(TB|TD|BT|RL)\b/gi, 'direction LR');
    ...
    try {
        await _loadMermaidScript();
        _initMermaidTheme();
        const id = `lumina-mermaid-${++_luminaMermaidIdCounter}`;

        // mermaid.render sometimes appends a dummy container (e.g., d<id>) to the body/document, clean it up
        const { svg } = await mermaid.render(id, source);
        wrapper.innerHTML = `<div class="lumina-mermaid-svg-container">${svg}</div>`;
        wrapper.classList.remove('is-loading');
        wrapper.classList.add('lumina-mermaid-rendered');

        const renderedSvg = wrapper.querySelector('svg');
        if (renderedSvg) {
            _alignEdgeLabelsWithClusters(renderedSvg);
        }

        // Remove any leftover element created by mermaid with id beginning with 'd' + id
        const dummyEl = document.getElementById(`d${id}`);
        if (dummyEl) {
            dummyEl.remove();
        }
    } catch (err) {
        // If it actually rendered successfully inside the wrapper despite a parse error log or warning,
        // we can check if wrapper's innerHTML contains SVG.
        if (wrapper.querySelector('svg')) {
            wrapper.classList.remove('is-loading');
            wrapper.classList.add('lumina-mermaid-rendered');
            const dummyEl = document.getElementById(`d` + `lumina-mermaid-${_luminaMermaidIdCounter}`);
            if (dummyEl) dummyEl.remove();
            return;
        }
        console.warn('[Lumina Mermaid] Render error:', err);
        const dummyEl = document.getElementById(`d` + `lumina-mermaid-${_luminaMermaidIdCounter}`);
        if (dummyEl) dummyEl.remove();
    }
}

// IntersectionObserver: render only when diagram scrolls into view
const _luminaMermaidIntersectionObserver = (typeof IntersectionObserver !== 'undefined')
    ? new IntersectionObserver((entries) => {
        for (const entry of entries) {
            if (entry.isIntersecting) {
                _luminaMermaidIntersectionObserver.unobserve(entry.target);
                _renderMermaidWrapper(entry.target);
            }
        }
    }, { threshold: 0.05, rootMargin: '120px 0px' })
    : null;

function processLuminaMermaidElements(rootNode) {
    if (!rootNode) return;
    const found = [];
    if (rootNode.classList && rootNode.classList.contains('lumina-mermaid-wrapper') &&
        !rootNode.classList.contains('is-loading-started')) {
        found.push(rootNode);
    }
    if (rootNode.querySelectorAll) {
        rootNode.querySelectorAll('.lumina-mermaid-wrapper:not(.is-loading-started)').forEach(el => found.push(el));
    }
    for (const wrapper of found) {
        if (_luminaMermaidIntersectionObserver) {
            _luminaMermaidIntersectionObserver.observe(wrapper);
        } else {
            // Fallback: render directly if IntersectionObserver not supported
            _renderMermaidWrapper(wrapper);
        }
    }
}
...
const luminaImageObserver = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
            ...
            processLuminaMermaidElements(node);
        }
    }
});
...
processLuminaMermaidElements(document.body);
...
class LuminaChatUI {
    ...
    _setupHistoryDelegation(historyEl = this.historyEl) {
        ...
        if (!this._historyDelegationClickHandler) {
            this._historyDelegationClickHandler = (e) => {
                ...
                // Click on rendered mermaid diagram → open in lightbox as SVG image
                const mermaidWrapper = e.target.closest('.lumina-mermaid-wrapper.lumina-mermaid-rendered');
                if (mermaidWrapper) {
                    e.stopPropagation();
                    const svgEl = mermaidWrapper.querySelector('svg');
                    if (svgEl) {
                        // Clone and ensure proper SVG namespace & sizing for preview
                        const clonedSvg = svgEl.cloneNode(true);
                        if (!clonedSvg.getAttribute('xmlns')) {
                            clonedSvg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
                        }
                        // Preserve computed width/height so preview renders at full size
                        const bbox = svgEl.getBoundingClientRect();
                        if (bbox.width > 0) clonedSvg.setAttribute('width', bbox.width);
                        if (bbox.height > 0) clonedSvg.setAttribute('height', bbox.height);

                        // Inject custom styles for the preview (which always has a white background)
                        const styleEl = document.createElementNS('http://www.w3.org/2000/svg', 'style');
                        styleEl.textContent = `
                            svg {
                                font-family: 'Google Sans', system-ui, -apple-system, sans-serif !important;
                            }
                            text, .node text, .edgeLabel text, span, p, div {
                                font-family: 'Google Sans', system-ui, -apple-system, sans-serif !important;
                                fill: #000000 !important;
                                color: #000000 !important;
                                font-size: 13px !important;
                            }
                            .edgeLabel p, .edgeLabel span, .edgeLabel div, .edgeLabel rect {
                        ...
                        // Use first line of mermaid source as caption
                        const rawDef = mermaidWrapper.getAttribute('data-mermaid') || '';
                        const caption = rawDef.split('\n')[0].trim() || 'Diagram';
    ...
    static applyViewportMinHeight(entry, container, inputWrapper) {
        ...
        if (viewportHeight > 0) {
            ...
            entry.style.setProperty('min-height', (viewportHeight - paddingOffset - 2) + 'px', 'important');
            return true;
        }
    ...
    _setupModelSelector(popup) {
        ...
        if (typeof window.getPromptApiSupport === 'undefined') {
            let promptApiPromise = null;
            window.getPromptApiSupport = function () {
                if (!promptApiPromise) {
                    promptApiPromise = (async () => {
                        try {
                            const ns = window.getPromptApiNamespace();
                            if (ns) {
                                const capabilities = await ns.capabilities();
                                if (capabilities && capabilities.available !== 'no') {
                                    if (capabilities.available === 'after-download') {
                                        ns.create().catch(e => console.warn("Auto-trigger download failed:", e));
                                    }
                                    return { supported: true, status: capabilities.available, reason: '' };
                                }
                            }
                        } catch (e) { }
                        try {
                            if (typeof window.ai !== 'undefined' && window.ai.canCreateTextSession) {
                                ...
                                if (availability !== 'no') {
                                    if (availability === 'after-download') {
                                        window.ai.createTextSession().catch(e => console.warn("Auto-trigger download failed:", e));
                                    }
                                    return { supported: true, status: availability, reason: '' };
                                }
                            }
                        } catch (e) { }
                        return { supported: false, status: 'no', reason: 'Hardware/browser does not support local Gemini Nano' };
                    })();
                }
                return promptApiPromise;
            };
        }
    ...
    static async processContainer(container) {
        ...
        processLuminaMermaidElements(container);
    }
