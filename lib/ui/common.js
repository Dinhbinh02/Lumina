...
class LuminaChatUI {
    ...
    _setupReasoningSelector(popup) {
        ...
        const render = (data) => {
            ...
            options.forEach((opt) => {
                ...
                el.onclick = (e) => {
                    ...
                    if (self.historyEl) {
                        self.historyEl.dispatchEvent(new CustomEvent('lumina:history-changed', {
                            bubbles: true,
                            detail: { force: true }
                        }));
                    }
        ...
        const fetchAndRenderReasoning = () => {
            const sid = self.historyEl?.dataset?.sessionId || null;
            const sidKey = sid || 'null';
            chrome.storage.local.get(['providers', 'advancedParamsByModel', 'lumina_session_settings'], (data) => {
                const settings = data.lumina_session_settings || {};
                const saved = settings[sidKey] || {};
                if (saved.thinkingLevel) {
                    self.thinkingLevel = saved.thinkingLevel;
                }
                render(data);
            });
        };
