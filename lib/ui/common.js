class LuminaChatUI {
    ...
    _updatePinnedQuestionChip() {
        ...
        if (!this._pinnedQuestionChipEl) {
            ...
            chip.innerHTML = '<span class="lumina-web-chip-label"></span>';
            this._pinnedQuestionChipEl = chip;
        }

        const labelEl = this._pinnedQuestionChipEl.querySelector('.lumina-web-chip-label');
        if (labelEl) {
            labelEl.textContent = chipLabel;
        }
        this._pinnedQuestionChipEl.title = chipLabel;
    ...
    gatherMessages(untilEntryId = null) {
        const messages = [];
        const entries = this.historyEl.querySelectorAll('.lumina-dict-entry');
                ...
                if (isProofreadEntry) {
                    ...
                    if (answerEl) {
                        const editableContent = answerEl.classList.contains('lumina-answer-editing')
                            ...
                            : answerEl;
                        proofreadAnswerText = (editableContent.innerText || editableContent.textContent || '').trim();
                    }
    ...
    _setupMicButton(btn, input) {
        ...
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (originalInputText && !originalInputText.endsWith(' ')) {
                    originalInputText += ' ';
                }
    ...
    _undoEditAndTruncate(entry, mode, questionDiv, answerDiv) {
        ...
        // Remove all sibling entries after the current one
        let next = entry.nextElementSibling;
        while (next) {
            const toRemove = next;
            next = next.nextElementSibling;
        ...
        if (mode === 'question' && questionDiv) {
            this.exitQuestionEditMode(questionDiv, false);
        } else if (mode === 'answer' && answerDiv) {
            this.exitAnswerEditMode(answerDiv, false);
        }

        // Apply min-height so it can scroll to top even if it's the last entry
        this.adjustEntryMargin(entry, 'immediate');

        // Scroll the entry to the top of the container
        requestAnimationFrame(() => {
