...
async function _renderMermaidWrapper(wrapper) {
    ...
    const source = (wrapper.getAttribute('data-mermaid') || '')
        .replace(/&amp;/g, '&')
        .replace(/&quot;/g, '"')
        .replace(/&#39;/g, "'")
        .replace(/&lt;/g, '<')
    ...
    try {
        ...
        const skeleton = wrapper.querySelector('.lumina-mermaid-skeleton');
        if (skeleton) {
            skeleton.textContent = 'Failed to render diagram';
            skeleton.style.background = '#fee2e2';
            skeleton.style.color = '#ea4335';
            skeleton.style.display = 'flex';
            skeleton.style.animation = 'none';
        }
        wrapper.classList.remove('is-loading');
