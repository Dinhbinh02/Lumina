class LuminaChatUI {
    ...
    constructor(container, options = {}) {
        ...
        this.systemTokens = 150; // Default estimate
        ...
        this._setupPinnedQuestionTracking();
    ...
    syncStateFromDOM() {
        ...
        if (entries.length > 0) {
            ...
            this.searchingDiv = lastEntry.querySelector('.lumina-searching-wrapper');

            // For older entries that might have been re-rendered, we find the answer container
    ...
    initListeners(container) {
        ...
        // Keep history delegation synced to the currently mounted history element.
        ...
        // Ensure all questions have pin buttons and proper row structure
        container.querySelectorAll('.lumina-chat-question').forEach(q => {
            this._attachQuestionPinButton(q);
        });
        ...
        container.addEventListener('keydown', (e) => {
            ...
            if (editable && e.target === editable) {
                ...
                if (e.key === 'Enter' && !e.shiftKey && !(e.ctrlKey || e.metaKey)) {
                    // Don't auto-submit on simple Enter if we're in the full editing mode
        ...
        // 3. Tooltip Delegation for Tags and Comments
        ...
        // 4. Mousedown Delegation (Open/focus Question/Answer edit mode)
        container.addEventListener('mousedown', (e) => {
            ...
            if (bubble) {
                ...
                if (editable) {
                    // If target is toolbar background or bubble background (not a button/link)
                    // and crucially, NOT the editable element itself, ensure it's focused.
    ...
    _setupHistoryDelegation(historyEl = this.historyEl) {
        ...
        if (!this._historyDelegationClickHandler) {
            this._historyDelegationClickHandler = (e) => {
                const pinButton = e.target.closest('.lumina-question-pin-btn');
                if (pinButton) {
                    const questionDiv = pinButton.closest('.lumina-question-row')?.querySelector('.lumina-chat-question');
                    if (questionDiv) {
                        e.preventDefault();
                        e.stopPropagation();
                        this._toggleQuestionPin(questionDiv);
                    }
                    return;
                }
    ...
    _setupPinnedQuestionTracking() {
        if (this._pinnedQuestionTrackingAttached) return;
        this._pinnedQuestionTrackingAttached = true;

        const scheduleUpdate = () => {
            if (this._pinnedQuestionScrollRaf) return;
            this._pinnedQuestionScrollRaf = requestAnimationFrame(() => {
                this._pinnedQuestionScrollRaf = null;
                this._updatePinnedQuestionChip();
            });
        };

        const bindScrollContainer = () => {
            const scrollContainer = this.getScrollContainer();
            if (!scrollContainer || this._pinnedQuestionScrollContainer === scrollContainer) return;

            if (this._pinnedQuestionScrollContainer && this._pinnedQuestionScrollHandler) {
                this._pinnedQuestionScrollContainer.removeEventListener('scroll', this._pinnedQuestionScrollHandler);
            }

            this._pinnedQuestionScrollContainer = scrollContainer;
            this._pinnedQuestionScrollHandler = scheduleUpdate;
            scrollContainer.addEventListener('scroll', this._pinnedQuestionScrollHandler, { passive: true });
            scheduleUpdate();
        };

        this._pinnedQuestionResizeHandler = scheduleUpdate;
        window.addEventListener('resize', this._pinnedQuestionResizeHandler, { passive: true });
        bindScrollContainer();
    ...
    _getQuestionPinButton(questionDiv) {
        return questionDiv?.closest('.lumina-question-row')?.querySelector('.lumina-question-pin-btn') || null;
    }
    ...
    _attachQuestionPinButton(questionDiv) {
        if (!questionDiv) return null;

        const row = this._ensureQuestionRow(questionDiv);
        if (!row) return null;

        const existingButton = this._getQuestionPinButton(questionDiv);
        const isPinned = this._pinnedQuestionEl === questionDiv;
        if (existingButton) {
            existingButton.classList.toggle('is-active', isPinned);
            existingButton.setAttribute('aria-pressed', isPinned ? 'true' : 'false');
            return existingButton;
        }

        const pinButton = document.createElement('button');
        pinButton.type = 'button';
        pinButton.className = 'lumina-question-pin-btn';
        pinButton.title = 'Pin question';
        pinButton.setAttribute('aria-label', 'Pin question');
        pinButton.setAttribute('aria-pressed', 'false');
        pinButton.innerHTML = '<span class="lumina-svg-icon lumina-icon-thumbtack" aria-hidden="true"></span>';
        ...
        row.insertBefore(pinButton, questionDiv);
        return pinButton;
    }

    _getPinnedQuestionSummary(questionDiv) {
        const rawText = String(questionDiv?.dataset?.rawText || questionDiv?.textContent || '').replace(/\s+/g, ' ').trim();
        if (!rawText) return 'Pinned question';
        return rawText.length > 56 ? `${rawText.slice(0, 53).trim()}...` : rawText;
    }

    _setPinnedQuestion(questionDiv) {
        if (!questionDiv) return;

        if (this._pinnedQuestionEl && this._pinnedQuestionEl !== questionDiv) {
            this._pinnedQuestionEl.classList.remove('is-pinned-question');
            const oldBtn = this._getQuestionPinButton(this._pinnedQuestionEl);
            if (oldBtn) {
        ...
        this._pinnedQuestionEl = questionDiv;
        questionDiv.classList.add('is-pinned-question');
        const pinButton = this._getQuestionPinButton(questionDiv);
        if (pinButton) {
    ...
    _clearPinnedQuestion() {
        if (this._pinnedQuestionEl) {
            this._pinnedQuestionEl.classList.remove('is-pinned-question');
            const pinButton = this._getQuestionPinButton(this._pinnedQuestionEl);
            if (pinButton) {
        ...
        this._pinnedQuestionEl = null;
        if (this._pinnedQuestionChipEl) {
            this._pinnedQuestionChipEl.remove();
            this._pinnedQuestionChipEl = null;
        }
        this._updateInputMetaVisibility();
    }

    _detachPinnedQuestionChip() {
        if (this._pinnedQuestionChipEl) {
            this._pinnedQuestionChipEl.remove();
            this._pinnedQuestionChipEl = null;
        }
    ...
    refreshPinnedQuestionState() {
        const historyEl = this.historyEl;
        if (!historyEl) {
            this._detachPinnedQuestionChip();
            return;
        }

        const allPinButtons = Array.from(historyEl.querySelectorAll('.lumina-question-row > .lumina-question-pin-btn'));
        const pinnedQuestion = historyEl.querySelector('.lumina-question-row > .lumina-chat-question.is-pinned-question') ||
            historyEl.querySelector('.lumina-chat-question.is-pinned-question');

        this._pinnedQuestionEl = pinnedQuestion || null;

        if (pinnedQuestion) {
            this._attachQuestionPinButton(pinnedQuestion);

            allPinButtons.forEach((btn) => {
                const btnQuestion = btn.closest('.lumina-question-row')?.querySelector('.lumina-chat-question');
                const isActive = btnQuestion === pinnedQuestion;
    ...
    _toggleQuestionPin(questionDiv) {
        if (!questionDiv) return;
        if (this._pinnedQuestionEl === questionDiv) {
            this._clearPinnedQuestion();
            return;
        }
        this._setPinnedQuestion(questionDiv);
    }

    _isPinnedQuestionVisible() {
        const questionDiv = this._pinnedQuestionEl;
        const scrollContainer = this.getScrollContainer();
        if (!questionDiv || !scrollContainer) return false;

        const containerRect = scrollContainer.getBoundingClientRect();
        const questionRect = questionDiv.getBoundingClientRect();
        return questionRect.bottom > containerRect.top + 24 && questionRect.top < containerRect.bottom - 24;
    }

    _scrollToPinnedQuestion() {
        if (!this._pinnedQuestionEl) return;
        this._scrollElementToTop(this._pinnedQuestionEl, 10);
    }

    _scrollElementToTop(element, topOffset = 0) {
        if (!element) return;
        const scrollContainer = this.getScrollContainer();
        if (!scrollContainer) return;
    ...
    _updatePinnedQuestionChip() {
        if (!this._pinnedQuestionEl || !this._pinnedQuestionEl.isConnected) {
            this._clearPinnedQuestion();
            return;
        }

        const chipLabel = 'Redirect';

        const isVisible = this._isPinnedQuestionVisible();
        if (isVisible) {
            if (this._pinnedQuestionChipEl) {
                this._pinnedQuestionChipEl.style.display = 'none';
            }
            this._updateInputMetaVisibility();
            return;
        }

        const redirectGroup = this._getRedirectChipsGroup();
        if (!redirectGroup) return;

        if (!this._pinnedQuestionChipEl) {
            const chip = document.createElement('button');
            chip.type = 'button';
            chip.className = 'lumina-web-chip lumina-pinned-question-chip';
            chip.title = 'Scroll to pinned question';
            chip.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                this._scrollToPinnedQuestion();
            });
            chip.innerHTML = `
            ...
            `;
            this._pinnedQuestionChipEl = chip;
        }

        this._pinnedQuestionChipEl.style.display = 'inline-flex';
        if (this._pinnedQuestionChipEl.parentElement !== redirectGroup) {
            redirectGroup.appendChild(this._pinnedQuestionChipEl);
        }
        this._updateInputMetaVisibility();
    }
    _updateInputMetaVisibility() {
        ...
        const isPinnedChipVisible = this._pinnedQuestionChipEl && this._pinnedQuestionChipEl.style.display !== 'none';
        const webGroup = this._getWebChipsGroup();
        const hasWebChips = webGroup && webGroup.children.length > 0;

        if (!isPinnedChipVisible && !hasWebChips) {
    ...
    appendQuestion(text, images = [], options = {}) {
        ...
        this._attachQuestionPinButton(questionDiv);
    ...
    createAnswerDiv() {
        ...
        const applyProofreadStyles = (div) => {
            // Apply a generic class for commenting system identification
    ...
    appendChunk(chunk, skipScroll = false) {
        ...
        if (!answerContentDiv.__isRich) {
            if (typeof marked !== 'undefined') {
                ...
                // Background fast-path fallback when markdown parser is unavailable
        ...
        if (!this._renderPending) {
            ...
            // Capture the target div in a local closure to avoid race conditions
            // if this.currentAnswerDiv is cleared (e.g., by an error or new request)
        ...
        if (thinkMatch) {
            ...
            if (thinkingContent) {
                ...
                if (!thinkingContainer) {
                    ...
                    `;
                    const header = thinkingContainer.querySelector('.lumina-thinking-header');
    ...
    appendError(text) {
        ...
        // Handle specific abortion error to make it more user-friendly
        if (text && (text.includes('BodyStreamBuffer') || text.includes('aborted'))) {
            div.classList.add('is-stopped');
            div.textContent = 'You stopped this response';
        } else {
            div.style.setProperty('color', 'var(--lumina-error)', 'important');
    ...
    getLoadingHTML() {
        return `<div class="lumina-thinking">Thinking</div>`;
    }
    showLoading(entryDiv = null) {
        ...
        const target = entryDiv || this.currentEntryDiv;
        if (target) {
            this.currentEntryDiv = target;
            const existingSep = target.querySelector(':scope > .lumina-dict-separator');
            if (existingSep) {
                target.insertBefore(this.loadingDiv, existingSep);
            } else {
    ...
    removeLoading() {
        if (this.loadingDiv && (!this.historyEl || !this.historyEl.contains(this.loadingDiv))) {
            this.syncStateFromDOM();
        }
        if (this.loadingDiv) {
        ...
        // Double check for any cloned loading wrappers that might have survived sync
        if (this.historyEl) {
            const extraLoading = this.historyEl.querySelector('.lumina-loading-wrapper');
            if (extraLoading) extraLoading.remove();
        }
    }
    clearAnswer(entryDiv) {
        if (!entryDiv) return;

        // Remove ALL existing answers and version containers
        ...
        const loading = entryDiv.querySelector('.lumina-loading-wrapper');
        if (loading) loading.remove();

        const searching = entryDiv.querySelector('.lumina-searching-indicator');
    ...
    removeSearching() {
        if (this.searchingDiv && (!this.historyEl || !this.historyEl.contains(this.searchingDiv))) {
            this.syncStateFromDOM();
        }
        ...
        // Double check for any cloned searching indicators that might have survived sync
        if (this.historyEl) {
            const extraSearching = this.historyEl.querySelector('.lumina-searching-indicator');
            if (extraSearching) extraSearching.remove();
        }
    }
    ...
    _clearCommentInteractions() {
        ...
        // 1. Remove all 'Send comment' buttons to prevent redundant submissions
        const buttons = this.historyEl.querySelectorAll('.lumina-send-comment-btn');
        buttons.forEach(btn => btn.remove());

        // Note: Highlights (.lumina-comment-highlight) are now PERSISTENT.
        // We no longer unwrap them here so they stay visible in history.
    }
    ...
    appendPartialTranslation(text) {
        ...
        `;
        this._attachQuestionPinButton(div.querySelector('.lumina-chat-question'));
        const scrollContainer = this.getScrollContainer();
        const preAppendScroll = scrollContainer ? scrollContainer.scrollTop : 0;
        this.historyEl.appendChild(div);
        this._throttledUpdateTokenCount();
    ...
    updatePartialTranslation(element, data) {
        ...
        `;
        this._attachQuestionPinButton(element.querySelector('.lumina-chat-question'));
        const separator = document.createElement('div');
        separator.className = 'lumina-dict-separator';
        ...
        if (!ignoreLimit && this.tokenLimit && this.tokenLimit > 0) {
            ...
            if (budget > 0) {
                ...
                for (let i = messages.length - 1; i >= 0; i--) {
                    const m = messages[i];
                    const mTokens = (typeof LuminaToken !== 'undefined') ? LuminaToken.count(m.text || '') : Math.ceil((m.text || '').length / 2); // Safer fallback
                ...
                // If context + input already exceeds limit, no history can be sent
                messages = [];
            }
        }
    ...
    _updateTokenCount() {
        ...
        // Attachment tokens (approx for text files)
        let attachmentTokens = 0;
        if (this.attachedFiles && this.attachedFiles.length > 0) {
            this.attachedFiles.forEach(file => {
                if (file.dataUrl && (file.mimeType?.startsWith('text/') || file.dataUrl.startsWith('data:text/'))) {
                    ...
                    // Images usually take around 256-1000 tokens depending on model
                    attachmentTokens += 765;
                }
            });
        }
        ...
        // Web Context tokens
        // Check for chips in the UI to get actual context tokens from data-tokens attribute
        let contextTokens = 0;
                ...
                <div class="lumina-input-bar" id="input-bar">
                    <div class="lumina-left-actions">
                         <button class="lumina-upload-btn" id="upload-btn" title="Upload File">
                            <svg viewBox="0 0 512 512" width="14" height="14"><path fill="currentColor" d="M490.667,234.667H277.333V21.333C277.333,9.551,267.782,0,256,0c-11.782,0-21.333,9.551-21.333,21.333v213.333H21.333   C9.551,234.667,0,244.218,0,256c0,11.782,9.551,21.333,21.333,21.333h213.333v213.333c0,11.782,9.551,21.333,21.333,21.333   c11.782,0,21.333-9.551,21.333-21.333V277.333h213.333c11.782,0,21.333-9.551,21.333-21.333   C512,244.218,502.449,234.667,490.667,234.667z"/></svg>
                         </button>
                         <div class="lumina-actions-dropdown-wrapper" id="tools-wrapper">
                              <button class="lumina-tools-toggle" id="tools-toggle">
                                 <svg viewBox="0 0 24 24" width="16" height="16" fill="none" style="opacity: 0.6;"><path d="M18 9l-6 6-6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" transform="rotate(180 12 12)"></path></svg>
                              ...
                              <div class="lumina-tools-dropdown" id="tools-dropdown">
                                  <div class="lumina-dropdown-header" style="padding: 8px 12px; font-size: 11px; font-weight: 700; color: var(--lumina-text-secondary); text-transform: uppercase; letter-spacing: 0.02em;">Tools</div>
                                      <button class="lumina-tool-item" data-action="proofread">
                                          <span class="lumina-svg-icon lumina-icon-text-check" aria-hidden="true"></span>
                                          <span>Proofread</span>
                                      </button>
                                      <button class="lumina-tool-item" data-action="translate">
                                          <img class="lumina-tool-translate-logo" src="${(typeof chrome !== 'undefined' && chrome.runtime && typeof chrome.runtime.getURL === 'function') ? chrome.runtime.getURL('assets/icons/lumina/Google_Translate_logo.svg.png') : 'assets/icons/lumina/Google_Translate_logo.svg.png'}" alt="" aria-hidden="true" />
                                          <span>Translate</span>
                                      </button>
                              </div>
                         </div>
                    ...
                    <div class="lumina-trailing-group">
                        <div class="lumina-token-counter" id="token-counter" title="Next request tokens (History + Context + Input)">
                            ...
                            <div class="lumina-token-details" id="token-details">
                                <div class="lumina-token-detail-item">
                                    ...
                                    <span class="detail-value lumina-token-system">0</span>
                                </div>
                                <div class="lumina-token-detail-item">
                                ...
                                <div class="lumina-token-detail-item">
                                    ...
                                    <span class="detail-value lumina-token-input">0</span>
                                </div>
                                <div class="lumina-token-divider"></div>
                                <div class="lumina-token-detail-item">
                                    <span class="detail-label">Limit</span>
                                    <input type="number" class="lumina-token-limit-input" placeholder="No limit" min="0" title="Auto-trim history to fit this limit">
                                </div>
                                <div class="lumina-token-detail-item lumina-token-total-row">
                                    <span class="detail-label">Total</span>
                                    <span class="detail-value lumina-token-total-full">0</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <button class="lumina-mic-btn" id="mic-btn" title="Voice Input">
                            <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm-1-9c0-.55.45-1 1-1s1 .45 1 1v6c0 .55-.45 1-1 1s-1-.45-1-1V5zm6 6c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>
                        </button>
                        <button class="lumina-action-btn" id="send-regen-btn">
                            <!-- Icon will be injected by _updateActionBtnState -->
                        </button>
                    </div>
                </div>
            </div>
    ...
    setupInputBar() {
        ...
        // Initial system tokens fetch
        this.refreshSystemTokens();

        // Listen for reasoning mode or language changes
        ...
        if (tokenCounter) {
            ...
            // Prevent clicks inside the details from closing it (e.g. clicking labels or the input)
        ...
        const checkExpand = () => {
            // No-op: System now uses a permanent 2-row grid layout
        ...
        const setupTool = (sel, toggleSel, modename, modeSetter, placeholder) => {
            ...
            if (item) item.addEventListener('click', (e) => {
                ...
                // Re-query toggle to ensure we have the live element in the current DOM
        ...
        const actionBtn = queryInPopup('#send-regen-btn');
        if (actionBtn) {
            actionBtn.addEventListener('click', (e) => {
                ...
                if (this.isGenerating) {
                    ...
                    const text = input.value.trim();
                    const hasHistory = this.historyEl && this.historyEl.querySelectorAll('.lumina-dict-entry').length > 0;

                    if (!text && hasHistory && !actionBtn.classList.contains('send')) {
                        // Regenerate last - Find last entry ID
                        const lastEntry = this.historyEl.querySelector('.lumina-dict-entry:last-child');
                        const entryId = lastEntry ? lastEntry.dataset.entryId : null;
                        this._handleSubmit(true, entryId);
                    } else if (text || this.attachedFiles.length > 0) {
                        this._handleSubmit();
                    }
                }
            });
        ...
        const render = (data) => {
            ...
            chain.forEach((item) => {
                ...
                el.onclick = (e) => {
                    ...
                    // Update model-specific token limit
    ...
    _setupMicButton(btn, input) {
        ...
        btn.addEventListener('click', async (e) => {
            ...
            try {
                // Ensure mic access before starting recognition to trigger option prompt if needed
                ...
                // Clear existing input as requested for a fresh start
                originalInputText = '';
                input.value = '';
                ...
                recognition.onresult = (event) => {
                    ...
                    if (finalTranscript) {
                        ...
                        // Capitalize only if we're starting a new sentence or the doc is empty
    ...
    _updateActionBtnState() {
        if (!this.container) return;
        const actionBtn = this.container.querySelector('#send-regen-btn') || document.querySelector('#send-regen-btn');
        if (!actionBtn) return;

        const val = this.inputEl ? this.inputEl.value.trim() : '';
        const hasHistory = this.historyEl && this.historyEl.querySelectorAll('.lumina-dict-entry').length > 0;

        actionBtn.className = 'lumina-action-btn';

        if (this.isGenerating) {
            actionBtn.classList.add('pause');
            actionBtn.innerHTML = `<svg viewBox="0 0 24 24" width="30" height="30" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>`;
            actionBtn.title = "Pause";
        } else if (val) {
            actionBtn.classList.add('active', 'send');
            actionBtn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg>`;
            actionBtn.title = "Send";
        } else if (hasHistory) {
            actionBtn.classList.add('regenerate');
            actionBtn.innerHTML = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M23 4v6h-6"></path><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path></svg>`;
    ...
    showStopButton(onStop = null) {
        ...
        // Legacy support for older UI layouts
        let stopBtn = null;
        if (this.container) {
    ...
    _initContextMenu() {
        ...
        this.contextMenu.innerHTML = `
            ...
            <button class="lumina-context-menu-item" data-action="copy-md">
                <span class="lumina-svg-icon lumina-icon-copy-alt" aria-hidden="true"></span>
                Copy Markdown
            </button>
        ...
        this.container.addEventListener('contextmenu', (e) => {
            if (this.historyEl?.querySelector('.lumina-answer-editing')) {
    ...
    _handleQuestionRecheck(userInput, editable, isRegenerate = false) {
        ...
        // Remove subsequents (Branch the conversation)
        ...
        // Remove the old answer and web-search artifacts
    ...
    enterQuestionEditMode(questionDiv) {
        ...
        toolbar.querySelector('.lumina-edit-undo').onclick = (e) => {
            ...
            this._undoEditAndTruncate(entry, 'question', questionDiv, null);
        };
        ...
        const keyHandler = (e) => {
            if (e.key === 'Escape') {
                ...
                // If just Enter (or Ctrl/Cmd+Enter) is pressed, it saves (matches main input behavior).
                // Shift+Enter will still insert a newline.
                e.preventDefault();
                this.exitQuestionEditMode(questionDiv, true);
                contentDiv.removeEventListener('keydown', keyHandler);
            }
    ...
    _undoEditAndTruncate(entry, mode, questionDiv, answerDiv) {
        ...
        // 1. Extract the question text from the entry before removing it
        const questionEl = entry.querySelector('.lumina-chat-question');
        // Prefer dataset.rawText as it is the original, unformatted input text
        let questionText = questionEl ? (questionEl.dataset.rawText || '') : '';

        if (!questionText && questionEl) {
            const qContent = questionEl.querySelector('.lumina-question-content') ||
                questionEl.querySelector('div[contenteditable="true"]') ||
                questionEl;

            // Fallback to innerText, cleaning up toolbars/buttons to avoid capturing their labels
            const temp = qContent.cloneNode(true);
            temp.querySelectorAll('.lumina-answer-edit-toolbar, .lumina-question-edit-toolbar, .lumina-edit-btn').forEach(el => el.remove());
            questionText = temp.innerText.trim();
        }

        // 2. Put the question text back into the main input
        // In Spotlight, history UI instances might not have inputEl set, so we look in the same pane
        const inputEl = this.inputEl ||
            (this.container && this.container.closest('.spotlight-pane')?.querySelector('.lumina-chat-input')) ||
        ...
        if (inputEl && questionText) {
            ...
            inputEl.focus();
            // Trigger layout update/auto-expand
            inputEl.style.height = 'auto';
            if (inputEl.scrollHeight > 0) {
                inputEl.style.height = inputEl.scrollHeight + 'px';
            }
            const expandFn = this._checkExpand || this.checkExpand;
            if (expandFn) expandFn();
        }

        // 3. Remove all sibling entries after the current one
        let next = entry.nextElementSibling;
        while (next) {
            const toRemove = next;
            next = next.nextElementSibling;
        ...
        // 4. Update currentEntryDiv and currentAnswerDiv to the previous entry
        ...
        // 5. Remove the entry itself
        entry.remove();

        // 6. Update UI state
        this._updateActionBtnState();

        // 7. Apply min-height and scroll to the new bottom if there's a previous entry
        if (prevEntry) {
            this.adjustEntryMargin(prevEntry, 'immediate');
            requestAnimationFrame(() => {
                const paddingTop = parseFloat(window.getComputedStyle(this.getScrollContainer()).paddingTop) || 0;
                this._scrollElementToTop(prevEntry, paddingTop);
            });
        }
    }
    ...
    exitQuestionEditMode(questionDiv, save = false) {
        ...
        if (save) {
            ...
            if (contentDiv) {
                // Remove extra padding/whitespace from the DOM visuals
    ...
    enterAnswerEditMode(answerDiv) {
        ...
        // Keep the rendered HTML in edit mode so markdown syntax is not exposed.
    ...
    serializeHistoryHTML() {
        ...
        // Strip transient min-height styles before saving to shared storage
