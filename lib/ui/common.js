        ...
        renderer: {
            image(token) {
                ...
                if (href && href.startsWith('image-search://')) {
                    const [searchUrl, hashPart] = href.split('#');
                    if (hashPart && hashPart.startsWith('data:')) {
                        return `<div class="lumina-image-wrapper"><img src="${hashPart}" data-original-href="${href}" alt="${text || 'diagram'}" class="lumina-async-image lumina-clickable-image" style="opacity: 1;" />${text ? `<div class="lumina-image-caption">${text}</div>` : ''}</div>`;
                    }
                    if (hashPart && hashPart.startsWith('cacheKey:')) {
                        const cacheKey = hashPart.substring('cacheKey:'.length);
                        return `<div class="lumina-image-wrapper is-loading"><div class="lumina-media-skeleton"></div><img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 600'%3E%3C/svg%3E" data-original-href="${href}" data-cache-key="${cacheKey}" alt="${text || 'diagram'}" class="lumina-async-image lumina-clickable-image" />${text ? `<div class="lumina-image-caption">${text}</div>` : ''}</div>`;
                    }

                    const query = searchUrl.replace('image-search://', '');
                    // Split keywords by space or plus, encode them, and join with commas
                    const keywords = query.split(/[\s\+]+/).filter(Boolean).map(encodeURIComponent);
                    // Take at most 3 keywords to ensure high match success rate in the general Flickr library
                    const safeQuery = keywords.slice(0, 3).join(',');
                    // Adding '/all' at the end is required to search the entire Flickr library instead of just cats
                    const primaryUrl = `https://loremflickr.com/800/600/${safeQuery}/all`;
                    // Use a transparent blank SVG data URL as src initially, storing target in data-src
                    return `<div class="lumina-image-wrapper is-loading"><div class="lumina-media-skeleton"></div><img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 600'%3E%3C/svg%3E" data-src="${primaryUrl}" data-query="${safeQuery}" data-original-href="${href}" alt="${text || 'diagram'}" class="lumina-async-image lumina-clickable-image" />${text ? `<div class="lumina-image-caption">${text}</div>` : ''}</div>`;
                }
                return false;
            },
            link(token) {
...
function compressAndSaveLuminaImage(img) {
    const originalHref = img.getAttribute('data-original-href');
    if (!originalHref) return;

    const answerDiv = img.closest('.lumina-chat-answer');
    if (!answerDiv) return;

    const performCompression = (imageSource, callback) => {
        const maxW = 1600;
        const maxH = 1200;
        let width = imageSource.naturalWidth || imageSource.width;
        let height = imageSource.naturalHeight || imageSource.height;
        if (!width || !height) {
            callback(null);
            return;
        }
        ...
        try {
            ...
            console.warn('[Lumina] Canvas drawing failed:', e);
            callback(null);
        }
    };

    const onCompressed = (compressedBase64) => {
        if (!compressedBase64) return;

        // Set the image src to the compressed base64
        img.src = compressedBase64;

        // Update raw text of the parent answer
        const rawText = answerDiv.getAttribute('data-raw-text') || '';
        if (rawText.includes(originalHref)) {
            const [baseUrl] = originalHref.split('#');
            const cacheKey = `img_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            const newHref = `${baseUrl}#cacheKey:${cacheKey}`;
            const newRawText = rawText.replaceAll(originalHref, newHref);
            answerDiv.setAttribute('data-raw-text', newRawText);
            img.setAttribute('data-original-href', newHref);
            img.setAttribute('data-cache-key', cacheKey);

            // Store the compressed base64 in local storage
            chrome.storage.local.set({ [`lumina_img_cache_${cacheKey}`]: compressedBase64 }, () => {
                // Trigger save to local storage
                const historyEl = img.closest('.lumina-chat-history') ||
                    img.closest('.lumina-chat-scroll-content') ||
    ...
    // Try direct canvas compression
    performCompression(img, (compressed) => {
        if (compressed) {
            onCompressed(compressed);
        } else {
            // Tainted canvas/CORS error. Request the background script to fetch it safely.
            if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
                chrome.runtime.sendMessage({
                    ...
                    if (response && response.success && response.dataUrl) {
                        ...
                        tempImg.onload = () => {
                            performCompression(tempImg, (compressed2) => {
                                if (compressed2) {
                                    onCompressed(compressed2);
                                }
                            });
                        };
                        tempImg.src = response.dataUrl;
                    } else {
                        console.warn('[Lumina] Background fetch_image_base64 failed:', response?.error);
                    }
                });
            }
        }
...
document.addEventListener('load', (event) => {
    ...
    if (target && target.tagName === 'IMG' && target.classList.contains('lumina-async-image') && !target.src.startsWith('data:')) {
        ...
        compressAndSaveLuminaImage(target);
    }
}, true);
...
document.addEventListener('error', (event) => {
    ...
    if (target && target.tagName === 'IMG' && target.classList.contains('lumina-async-image') && !target.src.startsWith('data:')) {
        ...
        if (fallbackUrls && fallbackUrls.length > 0) {
            ...
            const secondaryUrl = `https://picsum.photos/800/600?random=${Math.random()}`;
...
async function searchDuckDuckGoImages(query) {
    try {
        // Detect if query contains Vietnamese characters to prioritize fresh, local Vietnam region results
        const hasVietnamese = /[àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđ]/i.test(query);
        const regionParam = hasVietnamese ? '&kl=vn-vi' : '';

        const searchUrl = `https://duckduckgo.com/?q=${encodeURIComponent(query)}${regionParam}`;
        const mainRes = await fetch(searchUrl, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
        });
        if (!mainRes.ok) throw new Error('Failed to fetch DDG main page');
        const html = await mainRes.text();
        const vqdMatch = html.match(/vqd=['"]([^'"]+)['"]/);
        if (!vqdMatch) throw new Error('Could not find vqd token');

        const imagesApiUrl = `https://duckduckgo.com/i.js?q=${encodeURIComponent(query)}&o=json&vqd=${vqdMatch[1]}${regionParam}`;
        const apiRes = await fetch(imagesApiUrl, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
        });
        if (!apiRes.ok) throw new Error('Failed to fetch DDG images API');
        ...
        if (data && data.results && data.results.length > 0) {
            ...
            // Filter out small thumbnails, icons, avatars, and low-res images, plus photocard/shopping junk
            const filtered = data.results.filter(r => {
                if (!r.image) return false;
                // Exclude images under 400px width or 300px height
                if (r.width && r.width < 400) return false;
                if (r.height && r.height < 300) return false;
                // Exclude tiny icons/logos/avatars
                const imgLower = r.image.toLowerCase();
                if (imgLower.includes('icon') || imgLower.includes('avatar') || imgLower.includes('logo')) {
                    if (r.width && r.width < 600) return false;
                }

                const titleLower = (r.title || '').toLowerCase();
                const urlLower = (r.url || '').toLowerCase();

                // 1. Exclude shopping/auction/listing sites that host photocard trades/sales
                const shoppingSites = [
                    'ebay.com', 'shopee', 'lazada', 'mercari', 'amazon', 'etsy', 'aliexpress', 
                    'taobao', 'carousell', 'poshmark', 'depop', 'kpopstore', 'shop', 'store', 
                    'buy', 'price', 'listing', 'sales', 'gradedgem', 'psacard', 'cardboardconnection',
                    'tiki.vn', 'sendo.vn', 'tokopedia', 'bukalapak'
                ];
                if (shoppingSites.some(site => urlLower.includes(site) || imgLower.includes(site))) return false;

                // 2. Exclude typical card/graded/photocard/merchandise words, unless they were explicitly requested in the query
                const avoidWords = [
                    'photocard', 'photo card', 'graded card', 'graded-card', 'psa 10', 'psa 9', 'gem mt', 'bgs 10', 'gem mint', 
                    'graded', 'grade', 'grading', 'ebay', 'shopee', 'lazada', 'mercari', 'buy now',
                    'album cover', 'album-cover', 'albumcover', 'vinyl', 'cd', 'dvd', 'merch', 'merchandise',
                    'keyring', 'keychain', 'sticker', 'poster for sale', 'unboxing', 'packaging'
                ];
                for (const word of avoidWords) {
                    if (!queryLower.includes(word)) {
                        if (titleLower.includes(word) || imgLower.includes(word)) return false;
                    }
                }

                // 3. Year mismatches: if query contains a specific year (e.g. 2025), exclude results with a different year (e.g. 2023, 2024)
                if (queryYear) {
                    const titleYears = (titleLower.match(/\b(20\d{2})\b/g) || []).map(y => parseInt(y, 10));
                    const imgYears = (imgLower.match(/\b(20\d{2})\b/g) || []).map(y => parseInt(y, 10));
                    const allYears = [...new Set([...titleYears, ...imgYears])];
                    if (allYears.length > 0 && !allYears.includes(queryYear)) {
                        return false;
                    }
                }
            ...
            // Fallback to unfiltered list if everything was filtered out
            const finalResults = filtered.length > 0 ? filtered : data.results;
            return finalResults.map(r => r.image).filter(Boolean).slice(0, 4);
        }
    } catch (e) {
        console.warn('[Lumina] DuckDuckGo search error:', e);
    }
    return [];
}
...
const luminaImageObserver = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
            ...
            const imgs = [];
            if (node.classList && node.classList.contains('lumina-async-image') && !node.classList.contains('is-loading-started')) {
                imgs.push(node);
            }
            if (node.querySelectorAll) {
                const found = node.querySelectorAll('.lumina-async-image:not(.is-loading-started)');
                found.forEach(i => imgs.push(i));
            }
            ...
            imgs.forEach(async (img) => {
                ...
                // Reuse already running or finished search promises to avoid spamming network requests during stream renders
                ...
                const loadPromise = (async () => {
                    // Step 1: Try DuckDuckGo Image Search first (gives the most relevant real-world web images like Google Search)
                    try {
                        const urls = await searchDuckDuckGoImages(cleanQuery);
                        if (urls && urls.length > 0) {
                            return { url: urls[0], fallbackUrls: urls.slice(1) };
                        }
                    } catch (err) {
                        console.warn('[Lumina] DDG image search error, falling back to Wiki:', err);
                    }

                    // Step 2: Fallback to Wikimedia Commons for diagrams/illustrations
                    try {
                        const wikiUrl = `https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(cleanQuery)}&gsrnamespace=6&prop=imageinfo&iiprop=url&format=json&origin=*`;
                        ...
                        if (wikiRes.ok) {
                            ...
                            if (wikiData && wikiData.query && wikiData.query.pages) {
                                ...
                                const bestPage = pages.find(p => p.imageinfo && p.imageinfo[0] && p.imageinfo[0].url);
                                if (bestPage && bestPage.imageinfo[0].url) {
                                    return { url: bestPage.imageinfo[0].url, fallbackUrls: [] };
                                }
                            }
                        }
                    } catch (e) {
                        console.warn('[Lumina] Wikimedia Commons search failed:', e);
                    }

                    throw new Error('All image search options failed');
                })();
            ...
            // Process dynamic YouTube embeds
...
// Check immediately on load for pre-existing dynamic YouTube embeds
...
class LuminaChatUI {
    ...
    _setupPinnedQuestionTracking() {
        ...
        const scheduleUpdate = () => {
            ...
            if (this._pinnedQuestionDelayTimeout) clearTimeout(this._pinnedQuestionDelayTimeout);
            this._pinnedQuestionDelayTimeout = setTimeout(() => {
                this._updatePinnedQuestionChip();
            }, 250);
        };
        ...
        const bindScrollContainer = () => {
            ...
            if (this._pinnedQuestionResizeObserver) {
                this._pinnedQuestionResizeObserver.disconnect();
            }
            ...
            if (typeof ResizeObserver !== 'undefined') {
                this._pinnedQuestionResizeObserver = new ResizeObserver(() => {
                    scheduleUpdate();
                });
                this._pinnedQuestionResizeObserver.observe(scrollContainer);
            }
    ...
    refreshPinnedQuestionState() {
        ...
        allPinButtons.forEach((btn) => btn.remove());
        if (!pinnedQuestion) {
            this._detachPinnedQuestionChip();
        }
    ...
    getLoadingHTML() {
        return `<div class="lumina-thinking"><div class="lumina-dots-loader"><span></span><span></span><span></span></div></div>`;
    }
    ...
    showSearching() {
        ...
        this.searchingDiv.className = 'lumina-searching-indicator lumina-fetching-shimmer';
        this.searchingDiv.innerHTML = '<div class="lumina-loading-spinner"></div><span>Searching</span>';
        if (this.currentEntryDiv) {
    ...
    _setupReasoningSelector(popup) {
        ...
        const fetchAndRenderReasoning = () => {
            chrome.storage.local.get(['providers', 'advancedParamsByModel'], render);
        };
    ...
    enterQuestionEditMode(questionDiv) {
        ...
        toolbar.innerHTML = `
            ...
            <button class="lumina-edit-btn lumina-edit-pin${isPinned ? ' is-active' : ''}" title="${isPinned ? 'Unpin question' : 'Pin question'}">${isPinned ? 'Unpin' : 'Pin'}</button>
        ...
        const pinBtn = toolbar.querySelector('.lumina-edit-pin');
        if (pinBtn) {
            pinBtn.onclick = (e) => {
                e.preventDefault();
                e.stopPropagation();
                this._toggleQuestionPin(questionDiv);
                const currentlyPinned = this._pinnedQuestionEl === questionDiv;
                pinBtn.classList.toggle('is-active', currentlyPinned);
                pinBtn.textContent = currentlyPinned ? 'Unpin' : 'Pin';
                pinBtn.title = currentlyPinned ? 'Unpin question' : 'Pin question';
            };
        }
    ...
    static async processContainer(container) {
        ...
        // Resolve cached images from local storage
        const cachedImgs = Array.from(container.querySelectorAll('.lumina-async-image[data-cache-key]'));
        if (cachedImgs.length > 0) {
            await yieldToMain();
            const keysToFetch = cachedImgs.map(img => `lumina_img_cache_${img.dataset.cacheKey}`);
            await new Promise(resolve => {
                chrome.storage.local.get(keysToFetch, (data) => {
                    cachedImgs.forEach(img => {
                        const cacheKey = img.dataset.cacheKey;
                        const base64 = data[`lumina_img_cache_${cacheKey}`];
                        if (base64) {
                            img.src = base64;
                            img.style.opacity = '1';
                            ...
                            // Fallback: load the query via loremflickr if cache has expired or cleared
                            const originalHref = img.getAttribute('data-original-href') || '';
                            const [searchUrl] = originalHref.split('#');
                            const query = searchUrl.replace('image-search://', '');
                            const keywords = query.split(/[\s\+]+/).filter(Boolean).map(encodeURIComponent);
                            const safeQuery = keywords.slice(0, 3).join(',');
                            img.src = `https://loremflickr.com/800/600/${safeQuery}/all`;
                        }
                    });
                    resolve();
                });
    ...
    static injectAnswerActions(answerDiv) {
        if (!answerDiv || answerDiv.querySelector('.lumina-answer-actions')) return;
        ...
        actionsDiv.className = 'lumina-answer-actions';
        actionsDiv.innerHTML = `
            ...
            <button class="lumina-answer-action-btn" data-action="more" title="More options">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><circle cx="12" cy="12" r="2"/><circle cx="5" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg>
            </button>
        `;
