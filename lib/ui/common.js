...
class LuminaChatUI {
    ...
    showSearching(query) {
        ...
        this.searchingDiv.innerHTML = `<span>${text}</span>`;
        if (this.currentEntryDiv) {
            const questionDiv = this.currentEntryDiv.querySelector('.lumina-chat-question');
            const existingSep = this.currentEntryDiv.querySelector(':scope > .lumina-dict-separator');
            if (questionDiv) {
                // Insert after the question row wrapper (not the bubble itself) to avoid inline flex layout
                const questionRow = questionDiv.closest('.lumina-question-row') || questionDiv;
                questionRow.after(this.searchingDiv);
            } else if (existingSep) {
