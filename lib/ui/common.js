...
function _renderChartJSWrapper(wrapper) {
    ...
    // Validate JSON is complete and parseable before proceeding
    let config;
    try {
        config = JSON.parse(rawConfig);
    } catch (_) {
        // JSON not yet complete — silently wait for next update
        return;
    }

    // Mark as rendered to prevent re-triggering
    wrapper.setAttribute('data-last-rendered-source', configAttr);

    // Use rAF to avoid blocking the main thread
    requestAnimationFrame(() => {
        try {
            // Guard: Chart.js might still be loading (deferred script)
            ...
            // Circular charts (pie/doughnut/polarArea/radar) need a constrained aspectRatio
            // otherwise they expand to fill full height of the container
            const circularTypes = ['pie', 'doughnut', 'polarArea', 'radar'];
            const isCircular = circularTypes.includes(config.type);
            if (isCircular && config.options.aspectRatio === undefined) {
                config.options.aspectRatio = 2.2;
            }

            // Auto-fix draw order for mixed charts (bar + line):
            // In Chart.js v3, lower `order` = drawn LATER = appears on TOP.
            // Ensure line datasets always render above bar datasets.
            if (Array.isArray(config.data && config.data.datasets)) {
                const hasMixed = config.data.datasets.some(ds => ds.type === 'line') &&
                                 config.data.datasets.some(ds => !ds.type || ds.type === 'bar');
                if (hasMixed) {
            ...
            // Inject theme colors into all scale axes
            config.options.scales = config.options.scales || {};
            ['x', 'y', 'r'].forEach(axis => {
                if (config.options.scales[axis] !== undefined) {
            ...
            container.className = 'lumina-chartjs-container';
            const canvas = document.createElement('canvas');
            container.appendChild(canvas);

            // Destroy any stale chart instance
...
function processLuminaD2Elements(rootNode) {
    ...
    for (const wrapper of found) {
        if (wrapper.classList.contains('lumina-chartjs-wrapper')) {
            // Chart.js renders instantly, no need for IntersectionObserver
            _renderChartJSWrapper(wrapper);
