...
class LuminaHistory {
    ...
    init() {
        ...
        // Sync & Profile Integration
        this.syncProfileContainer = document.getElementById('lumina-history-sync-profile');
            ...
            const renderSyncUI = (isAuthenticated, user) => {
                if (!isAuthenticated) {
                    this.syncProfileContainer.innerHTML = `
                        <button class="m3-btn m3-btn-filled w-100" id="sidebar-login-btn" style="height: 36px;">
                            Sign in with Google
                        </button>
                    `;
                    ...
                    this.syncProfileContainer.innerHTML = `
                        <div class="sync-profile" style="display: flex; align-items: center; justify-content: space-between; gap: 8px; width: 100%; padding: 8px 0; border-bottom: 1px solid var(--lumina-ui-border, rgba(0,0,0,0.06));">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <img src="${avatarUrl}" alt="Avatar" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;">
                                <div style="display: flex; flex-direction: column;">
                                    <span style="font-size: 13px; font-weight: 550; color: var(--lumina-text-primary, #1f1f1f); max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${userName}</span>
                                    <span id="sidebar-sync-status" style="font-size: 11px; color: var(--lumina-text-secondary, #70757a);">Checking sync...</span>
                                </div>
                            </div>
                            <div style="display: flex; gap: 4px; align-items: center;">
                                <button id="sidebar-sync-now-btn" class="m3-icon-button" title="Sync Now" style="width: 28px; height: 28px; border-radius: 50%; border: none; background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--lumina-text-secondary, #70757a);">
                                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l6.07-5.19"/></svg>
                                </button>
                                <button id="sidebar-logout-btn" class="m3-icon-button" title="Sign Out" style="width: 28px; height: 28px; border-radius: 50%; border: none; background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--md-sys-color-error, #b3261e);">
