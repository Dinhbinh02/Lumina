...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderNoteTypeDropdown() {
        ...
        if (STATE.models.length === 0) {
            noteTypeDropdown.innerHTML = '<div style="padding:8px;font-size:11px;color:#999;">No Anki Models found</div>';
            return;
        }
    ...
    function renderModelDropdown(dropdown, labelEl, type) {
        ...
        if (STATE.aiModelChains.length === 0) {
            dropdown.innerHTML = '<div style="padding:8px;font-size:11px;color:#999;">No AI models configured</div>';
            return;
        }
        ...
        STATE.currentEditTemplate.fields.forEach((field, index) => {
            ...
            if (isInput) {
                setup.innerHTML = `
                    ...
                    <div style="font-size:11px; color:#888; margin-top:4px;">This field automatically uses the input word and cannot be edited.</div>
                `;
            } else {
                setup.innerHTML = `
    ...
    async function openExampleModal(index = -1) {
        ...
        if (isNew) {
            ...
            loadingBar.innerHTML = `<div style="display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 620;"><div class="spinner-small"></div> Generating example...</div>`;
